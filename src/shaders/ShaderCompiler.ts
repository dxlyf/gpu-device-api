...
/** 默认精度前言（不含版本指令）。 */
export const GLSL_PRECISION_PREAMBLE =
 ...
 */
export function resolveGlslWrapOptions(options?: GlslWrapOptions): ResolvedGlslWrapOptions {
 ...
 */
export function wrapGlslSource(
  body: string,
  defines?: ShaderCompileRequest['defines'],
  label = 'shader',
  options?: GlslWrapOptions,
): string {
  const { version, preamble } = resolveGlslWrapOptions(options);
  const defineBlock = glslDefines(defines);

  // 全部关掉：源码原样交给后端，本库一个字符都不改。
  if (!version && !preamble && !defineBlock) return body;
  ...
  if (defineBlock) parts.push(`${defineBlock}\n`);
