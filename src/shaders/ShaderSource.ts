...
/** 每种后端使用的着色器语言是固定的。 */
export function languageForBackend(backend: 'webgl2' | 'webgpu'): ShaderLanguage {
  return backend === 'webgl2' ? 'glsl' : 'wgsl';
}

/** 某个 stage 对应的 GLSL 源码在 {@link ShaderSource} 里的字段名。 */
export function glslFieldForStage(stage: ShaderStage): 'vs' | 'fs' | 'cs' | null {
  if (stage === 0x0001) return 'vs';
  if (stage === 0x0002) return 'fs';
  if (stage === 0x0004) return 'cs';
  return null;
}
...
export function describeShaderSource(source: ShaderSource): string {
  const parts: string[] = [];
  if (source.vs) parts.push('vs（GLSL）');
  if (source.fs) parts.push('fs（GLSL）');
  if (source.cs) parts.push('cs（GLSL）');
 ...
 */
export function missingSourceMessage(
  ...
  return (
    `[gpu-device-api] ShaderModule「${label}」缺少 ${backend} 后端需要的 ${stageName} 阶段源码。\n` +
    `  ${expected}；\n` +
