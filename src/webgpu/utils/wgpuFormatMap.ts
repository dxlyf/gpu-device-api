...
const GPU_FORMATS: Readonly<Record<TextureFormat, GPUTextureFormat>> = Object.freeze({
  // 8 位单通道
  r8unorm: 'r8unorm',
  r8snorm: 'r8snorm',
  r8uint: 'r8uint',
...
function caps(
  ...
  const sampleScalar: TextureFormatCapabilities['sampleScalar'] =
    kind === 'depth' || kind === 'stencil' ? 'depth' : kind === 'uint' ? 'uint' : kind === 'sint' ? 'sint' : 'float';
  return {
...
/** 全部 core 格式的能力表。 */
export const TEXTURE_FORMAT_CAPABILITIES: Readonly<Record<TextureFormat, TextureFormatCapabilities>> =
  Object.freeze({
...
export function textureFormatCapabilities(format: TextureFormat): TextureFormatCapabilities {
  const capabilities = TEXTURE_FORMAT_CAPABILITIES[format];
  if (!capabilities) {
    throw new ValidationError(
      `[gpu-device-api] Unknown or unsupported TextureFormat "${String(format)}".`,
    );
  }
  return capabilities;
}
...
export function fromGPUTextureFormat(format: string): TextureFormat {
  for (const [core, gpu] of Object.entries(GPU_FORMATS) as [TextureFormat, GPUTextureFormat][]) {
    if (gpu === format) return core;
  }
  throw new ValidationError(
    `[gpu-device-api] GPU texture format "${format}" has no core TextureFormat counterpart ` +
      '(compressed formats and the extra WebGPU-only formats are not part of core).',
  );
}

/** 是否含有 depth aspect。 */
export function hasDepthAspect(format: TextureFormat): boolean {
...
export function isDepthOrStencilFormat(format: TextureFormat): boolean {
  return isDepthStencilFormat(format);
}

/** 线性过滤是否需要额外的 feature。 */
export function filterFeatureFor(format: TextureFormat): string | undefined {
  return textureFormatCapabilities(format).filterFeature;
}

/** 在给定设备能力下是否可被线性过滤。 */
export function isFilterableFormat(format: TextureFormat, features?: DeviceFeatures): boolean {
  const capabilities = textureFormatCapabilities(format);
  if (capabilities.filterable) return true;
  if (!capabilities.filterFeature) return false;
  return features ? features.has(capabilities.filterFeature) : false;
}

/** 在给定设备能力下是否可作为 render attachment（color 或 depth/stencil）。 */
export function isRenderableFormat(format: TextureFormat, features?: DeviceFeatures): boolean {
  const capabilities = textureFormatCapabilities(format);
  if (capabilities.renderable || capabilities.depthStencilAttachment) return true;
  if (!capabilities.renderFeature) return false;
  return features ? features.has(capabilities.renderFeature) : false;
}

/** 在给定设备能力下是否可作为 storage texture。 */
export function isStorageTextureFormat(format: TextureFormat, features?: DeviceFeatures): boolean {
  const capabilities = textureFormatCapabilities(format);
  if (!capabilities.storage && !capabilities.storageFeature) return false;
  if (!capabilities.storageFeature) return true;
  return features ? features.has(capabilities.storageFeature) : false;
}

/** 断言格式可作为 render attachment（color 或 depth/stencil）。 */
export function assertRenderableFormat(
  format: TextureFormat,
  features: DeviceFeatures | undefined,
  context: string,
): void {
  const capabilities = textureFormatCapabilities(format);
  if (capabilities.renderable || capabilities.depthStencilAttachment) return;
  if (capabilities.renderFeature) {
    const enabled = features ? features.has(capabilities.renderFeature) : false;
    if (enabled) return;
    throw new ValidationError(
      `[gpu-device-api] ${context}: "${format}" requires the "${capabilities.renderFeature}" device feature ` +
        'to be used as a render attachment (it is not enabled on this device).',
    );
  }
  throw new ValidationError(
    `[gpu-device-api] ${context}: "${format}" cannot be used as a render attachment in WebGPU ` +
      '(snorm color formats and rgb9e5ufloat have no renderable support).',
  );
}

/** 断言格式可以作为 `TextureBinding` 被采样。 */
export function assertSampleableFormat(format: TextureFormat, context: string, multisampled = false): void {
  const capabilities = textureFormatCapabilities(format);
  if (multisampled) {
    throw new ValidationError(
      `[gpu-device-api] ${context}: a multisampled texture ("${format}", sampleCount > 1) cannot be used as a ` +
        'TextureBinding; WebGPU only allows multisampled textures as render attachments.',
    );
  }
  if (capabilities.sampleable) return;
  if (capabilities.kind === 'stencil') {
    throw new ValidationError(
      `[gpu-device-api] ${context}: "stencil8" has no sampleable aspect; bind a depth format instead.`,
    );
  }
  throw new ValidationError(
    `[gpu-device-api] ${context}: "${format}" can only be used as a depth/stencil attachment and cannot be ` +
...
export function assertStorageTextureFormat(
  format: TextureFormat,
  features: DeviceFeatures | undefined,
  context: string,
): void {
  ...
  if (capabilities.storageFeature) {
    if (features?.has(capabilities.storageFeature)) return;
    throw new ValidationError(
      `[gpu-device-api] ${context}: "${format}" requires the "${capabilities.storageFeature}" device feature ` +
        'to be used as a storage texture (it is not enabled on this device).',
    );
  }
  throw new ValidationError(
    `[gpu-device-api] ${context}: "${format}" cannot be used as a storage texture. WebGPU only allows ` +
...
export function assertCopyableFormat(format: TextureFormat, context: string): void {
  const capabilities = textureFormatCapabilities(format);
  if (capabilities.copyable) return;
  throw new ValidationError(
    `[gpu-device-api] ${context}: "${format}" is not copy-compatible in WebGPU (its memory layout is ` +
      'implementation defined), so it cannot be used with CopySrc/CopyDst. Use "depth32float" for depth readback.',
  );
}

/** 断言 aspect 与格式匹配。 */
export function assertTextureAspectForFormat(format: TextureFormat, aspect: TextureAspect, context: string): void {
  if (aspect === 'all') return;
  if (aspect === 'depth-only' && !hasDepthAspect(format)) {
    throw new ValidationError(
      `[gpu-device-api] ${context}: aspect "depth-only" is invalid for format "${format}", which has no depth aspect.`,
    );
  }
  if (aspect === 'stencil-only' && !hasStencilAspect(format)) {
    throw new ValidationError(
      `[gpu-device-api] ${context}: aspect "stencil-only" is invalid for format "${format}", which has no stencil aspect.`,
    );
  }
}

/** 从格式推导 WebGPU texture binding 的默认 sampleType。 */
export function defaultTextureSampleType(format: TextureFormat): GPUTextureSampleType {
  const capabilities = textureFormatCapabilities(format);
...
export function assertTextureUsageSupported(
  format: TextureFormat,
  usage: TextureUsage,
  options: {
    sampleCount?: number;
  ...
  if (sampleCount > 1) {
    ...
    if ((options.mipLevelCount ?? 1) > 1) {
      throw new ValidationError(
        `[gpu-device-api] ${context}: a multisampled texture must have exactly one mip level.`,
      );
    }
    if (options.dimension !== undefined && options.dimension !== '2d') {
      throw new ValidationError(
        `[gpu-device-api] ${context}: a multisampled texture must be "2d", got "${options.dimension}".`,
      );
    }
    if ((usage & (TextureUsageFlags.CopySrc | TextureUsageFlags.CopyDst)) !== 0) {
      throw new ValidationError(
        `[gpu-device-api] ${context}: a multisampled texture cannot be a copy source or destination; ` +
    ...
    if ((usage & (TextureUsageFlags.TextureBinding | TextureUsageFlags.StorageBinding)) !== 0) {
      throw new ValidationError(
        `[gpu-device-api] ${context}: a multisampled texture can only be used as a render attachment.`,
      );
    }
  }
}
