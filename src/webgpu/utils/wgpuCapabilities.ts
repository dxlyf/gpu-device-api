...
] as const satisfies readonly (keyof DeviceLimits)[];
 ...
 */
export const FALLBACK_DEVICE_LIMITS: DeviceLimits = Object.freeze({
...
export function getWebGPU(): GPU | null {
  const navigatorLike = (globalThis as { navigator?: { gpu?: unknown } }).navigator;
  const gpu = navigatorLike?.gpu;
  if (!gpu || typeof (gpu as { requestAdapter?: unknown }).requestAdapter !== 'function') return null;
  return gpu as GPU;
}
...
export async function requestWebGPUAdapter(
  options: WebGPUAdapterRequestOptions = {},
): Promise<GPUAdapter | null> {
  const gpu = getWebGPU();
  if (!gpu) return null;
  const request: GPURequestAdapterOptions = {};
  ...
  if (options.featureLevel !== undefined) request.featureLevel = options.featureLevel;
  if (options.xrCompatible !== undefined) request.xrCompatible = options.xrCompatible;
  return gpu.requestAdapter(request);
}

/** 读取 `GPUSupportedFeatures` 的集合形态；实现缺失时返回空集合。 */
export function readSupportedFeatures(features: GPUSupportedFeatures | undefined): ReadonlySet<string> {
  const result = new Set<string>();
  if (!features) return result;
  const iterable = features as unknown as { [Symbol.iterator]?: () => Iterator<string> };
  if (typeof iterable[Symbol.iterator] === 'function') {
    for (const feature of features as unknown as Iterable<string>) result.add(feature);
    return result;
  }
  const keys = (features as unknown as { keys?: () => Iterable<string> }).keys;
  if (typeof keys === 'function') {
    for (const feature of keys.call(features)) result.add(feature);
    return result;
  }
  const forEach = (features as unknown as { forEach?: (cb: (value: string) => void) => void }).forEach;
  if (typeof forEach === 'function') {
    forEach.call(features, (value: string) => result.add(value));
  }
  return result;
}

/** 读取 `GPUSupportedLimits`；缺失的字段用 {@link FALLBACK_DEVICE_LIMITS} 补齐。 */
export function readDeviceLimits(limits: GPUSupportedLimits | undefined): DeviceLimits {
  const source = (limits ?? {}) as unknown as Record<string, unknown>;
  const resolved = {} as Record<keyof DeviceLimits, number>;
  ...
  return resolved as DeviceLimits;
}

/** 把原生 adapter 信息读成 core 的 {@link AdapterInfo}。 */
export function describeWebGPUAdapter(adapter: GPUAdapter): AdapterInfo {
  ...
  for (const feature of required) {
    ...
    if (!available.has(feature)) {
      throw new ValidationError(
        `[gpu-device-api] ${context}: the adapter does not support the "${feature}" feature. ` +
...
export class WebGPUFeatures implements DeviceFeatures {
...
export function preferredCanvasFormat(): TextureFormat {
  const gpu = getWebGPU();
  if (!gpu) return 'bgra8unorm';
...
/** 校验并转发 canvas 格式。 */
export function toCanvasFormat(format: TextureFormat): GPUTextureFormat {
  const gpuFormat = toGPUTextureFormat(format);
  if (gpuFormat !== 'rgba8unorm' && gpuFormat !== 'bgra8unorm') {
    throw new ValidationError(
      `[gpu-device-api] CanvasContext.configure: WebGPU only allows "rgba8unorm" or "bgra8unorm" as the ` +
...
export function getWebGPUCanvasContext(
  ...
  const getContext = canvas.getContext as (contextId: string) => unknown;
  const context = getContext.call(canvas, 'webgpu');
  if (!context || typeof (context as GPUCanvasContext).getCurrentTexture !== 'function') return null;
  return context as GPUCanvasContext;
}

/** 该 canvas 元素是否支持 WebGPU context。 */
export function isWebGPUCanvasSupported(canvas: HTMLCanvasElement | OffscreenCanvas): boolean {
