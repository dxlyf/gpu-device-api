...
function asWebGPU(device: Device, context: string): void {
  if (device.backend === 'webgpu') return;
  throw new ValidationError(
    `[gpu-device-api] ${context}: expected a WebGPU device, got backend "${device.backend}".`,
  );
}
 ...
 */
export async function prewarmWebGPURenderPipeline(
  device: Device,
  descriptor: RenderPipelineDescriptor,
  variant: Partial<RenderPipelineVariant> = {},
  options: PrewarmOptions = {},
): Promise<RenderPipelinePrewarmResult> {
  asWebGPU(device, 'prewarmWebGPURenderPipeline');
  const pipeline = device.createRenderPipeline(descriptor);
  if (!(pipeline instanceof WebGPURenderPipeline)) {
    throw new ValidationError(
      '[gpu-device-api] prewarmWebGPURenderPipeline: createRenderPipeline() returned a pipeline that is ' +
...
export async function prewarmWebGPUComputePipeline(
  device: Device,
  descriptor: ComputePipelineDescriptor,
  options: PrewarmOptions = {},
): Promise<ComputePipelinePrewarmResult> {
  ...
  if (!(pipeline instanceof WebGPUComputePipeline)) {
    throw new ValidationError(
      '[gpu-device-api] prewarmWebGPUComputePipeline: createComputePipeline() returned a pipeline that is ' +
