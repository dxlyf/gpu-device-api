...
/** 网格生成过程中的可变累加器，最后一次性转成 typed array。 */
...
function createMesh(): MeshAccumulator {
  return { position: [], normal: [], uv: [], index: [] };
}

function createLines(): LineAccumulator {
...
function toShapeData(mesh: MeshAccumulator): ShapeData {
  return {
...
function toLineData(lines: LineAccumulator): LineData {
  return {
...
function vertexCount(mesh: MeshAccumulator): number {
...
function assertSegmentCount(value: number, minimum: number, name: string): number {
...
/** 默认线段颜色（中灰）。 */
const DEFAULT_LINE_COLOR: readonly [number, number, number, number] = [0.5, 0.5, 0.5, 1];
 ...
 * 因此从 +Z 方向看过去是 CCW。
 */
export function createTriangle(options: TriangleOptions = {}): ShapeData {
 ...
 */
export function createPlane(options: PlaneOptions = {}): ShapeData {
 ...
 */
export function createBox(options: BoxOptions = {}): ShapeData {
  const width = assertFinitePositive(options.width ?? 1, 'options.width');
  const height = assertFinitePositive(options.height ?? 1, 'options.height');
 ...
 * 因此两极法线不会出现 NaN。
 */
export function createSphere(options: SphereOptions = {}): ShapeData {
  const radius = assertFinitePositive(options.radius ?? 0.5, 'options.radius');
 ...
 */
export function createTorus(options: TorusOptions = {}): ShapeData {
  const radius = assertFinitePositive(options.radius ?? 0.5, 'options.radius');
 ...
 */
function pushCaps(mesh: MeshAccumulator, y: number, radius: number, outwardY: 1 | -1, radialSegments: number): void {
 ...
 */
export function createCylinder(options: CylinderOptions = {}): ShapeData {
  ...
  if (caps) {
    if (radiusTop > 0) pushCaps(mesh, height / 2, radiusTop, 1, radialSegments);
    if (radiusBottom > 0) pushCaps(mesh, -height / 2, radiusBottom, -1, radialSegments);
  }
 ...
 */
export function createCone(options: ConeOptions = {}): ShapeData {
  const radius = assertFinitePositive(options.radius ?? 0.5, 'options.radius');
 ...
 */
export function createGrid(options: GridOptions = {}): LineData {
 ...
 */
export function createAxes(options: AxesOptions = {}): LineData {
