...
export class PerspectiveCamera {
  ...
  private readonly projectionBuffer: Mat4 = mat4.create();
  /** 唯一的那份投影视图矩阵缓冲（对外通过 {@link projectionViewMatrix} 读取）。 */
  private readonly projectionViewBuffer: Mat4 = mat4.create();
  /** `depthRange` 的实际存储；见 {@link PerspectiveCamera.depthRange} 的说明。 */
  private _depthRange: DepthRangeConvention;
  /**
   * 上一次算投影矩阵用的是哪种深度约定；与 `_depthRange` 不一致就说明缓存失效
   * （读取时由 {@link PerspectiveCamera.ensureProjection} 重算，而不是把旧的读出去）。
   */
  private projectedDepthRange: DepthRangeConvention | null = null;
  ...
  set depthRange(value: DepthRangeConvention) {
    if (value === this._depthRange) return;
  ...
  private ensureProjection(): void {
    if (this.projectedDepthRange === this._depthRange) return;
    this.computeProjection();
  }
}
