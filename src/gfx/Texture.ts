...
interface UploadFormat {
  ...
  /** 颜色分量数。gfx 目前只支持 1 / 2 / 4 通道的 8 位格式。 */
 ...
 */
const UPLOAD_FORMATS: Readonly<Partial<Record<TextureFormat, UploadFormat>>> = Object.freeze({
...
export const GFX_UPLOAD_FORMATS: readonly TextureFormat[] = Object.freeze(
  Object.keys(UPLOAD_FORMATS) as TextureFormat[],
);
 ...
 */
const WEBGL2_UNSUPPORTED_FORMATS: ReadonlySet<string> = new Set(['bgra8unorm', 'bgra8unorm-srgb']);

/** 解析并校验格式；不支持的格式给出可直接照做的替代方案。 */
function resolveUploadFormat(format: TextureFormat, device: Device): UploadFormat {
  const info = UPLOAD_FORMATS[format];
  if (!info) {
    throw new ValidationError(
      `[gpu-device-api] The gfx texture layer cannot upload "${format}" from host memory. ` +
  ...
  if (device.backend === 'webgl2' && WEBGL2_UNSUPPORTED_FORMATS.has(format)) {
    throw new ValidationError(
      `[gpu-device-api] Texture format "${format}" has no WebGL2 equivalent: BGRA exists there only ` +
...
export class GfxTexture {
    ...
    return GfxTexture.build(device, {
      ...
      upload: (texture) => {
        device.queue.writeTexture(
          ...
          decoded.data,
          { offset: 0, bytesPerRow: decoded.width * info.bytesPerPixel, rowsPerImage: decoded.height },
   ...
   */
  static async fromImage(device: Device, desc: AsyncTextureDesc): Promise<GfxTexture> {
    const label = desc.label ?? 'texture';
    const format = desc.format ?? 'rgba8unorm';
    const info = resolveUploadFormat(format, device);
    ...
    try {
      ...
      if (width <= 0 || height <= 0) {
        throw new ValidationError(
          `[gpu-device-api] GfxTexture.fromImage("${label}"): the decoded image is ${width}x${height}.`,
        );
      }
      if ((desc.width !== undefined && desc.width !== width) || (desc.height !== undefined && desc.height !== height)) {
        throw new ValidationError(
          `[gpu-device-api] GfxTexture.fromImage("${label}"): width/height (${String(desc.width)}x${String(desc.height)}) ` +
      ...
      return GfxTexture.build(device, {
        ...
        upload: (texture) => {
          device.queue.writeTexture(
            { texture, mipLevel: 0, origin: { x: 0, y: 0, z: 0 } },
            decoded.data,
            { offset: 0, bytesPerRow: width * info.bytesPerPixel, rowsPerImage: height },
            { width, height, depthOrArrayLayers: 1 },
          );
        },
      });
   ...
   */
  private static build(device: Device, init: BuildInit): GfxTexture {
    ...
    try {
      init.upload(texture);
      if (useRenderAttachment) generateTextureMipmaps(texture, init.label, device.backend);
...
interface BuildInit {
  ...
  readonly wrapS: AddressMode;
  readonly wrapT: AddressMode;
  /** 除默认位之外还必须带上的 usage（例如 `copyExternalImageToTexture` 要求的 RenderAttachment）。 */
  readonly extraUsage?: TextureUsage;
  readonly upload: (texture: Texture) => void;
}
 ...
 */
function generateTextureMipmaps(texture: Texture, label: string, backend: string): void {
  const generate = texture.generateMipmaps;
  if (typeof generate !== 'function') {
    throw new ValidationError(
      `[gpu-device-api] Texture "${label}": the ${backend} backend does not implement ` +
...
function decodeSource(
  ...
  if (!isImageSource(source)) {
    ...
    if (w <= 0 || h <= 0) {
      throw new ValidationError(
        '[gpu-device-api] Creating a texture from raw pixels requires explicit width and height ' +
    ...
    if (bytes.byteLength < expected) {
      throw new ValidationError(
        `[gpu-device-api] Texture data has ${bytes.byteLength} bytes, but a ${w}x${h} ${format} ` +
  ...
  if (typeof document === 'undefined' && typeof OffscreenCanvas === 'undefined') {
    throw new ValidationError(
      '[gpu-device-api] There is no canvas in this environment, so the image source cannot be ' +
  ...
  if (w <= 0 || h <= 0) {
    throw new ValidationError(
      '[gpu-device-api] The image source has no size yet (the image probably has not finished ' +
...
function isImageData(value: unknown): value is ImageData {
  return (value as { constructor?: { name?: string } }).constructor?.name === 'ImageData';
}
...
function packFromRgba(
  ...
  if (rgba.byteLength < pixelCount * 4) {
    throw new ValidationError(
      `[gpu-device-api] Expected at least ${pixelCount * 4} RGBA bytes for a ${width}x${height} image, ` +
        `got ${rgba.byteLength}.`,
    );
  }
  ...
  if (format === 'r8unorm') {
    for (let index = 0; index < pixelCount; index++) out[index] = rgba[index * 4]!;
    return out;
  }
  if (format === 'rg8unorm') {
...
/** 逐行上下翻转（图像坐标 → GL 纹理坐标）。 */
function flipRows(data: Uint8Array, width: number, height: number, bytesPerPixel: number): Uint8Array {
...
async function loadImageBitmap(
  ...
  if (typeof createImageBitmap !== 'function') {
    throw new ValidationError(
      '[gpu-device-api] GfxTexture.fromImage requires createImageBitmap, which this environment ' +
        'does not provide; use GfxTexture.create() with raw pixels or an ImageData source instead.',
    );
  }
  const resolved: ImageBitmapOptions = {
  ...
  if (typeof source === 'string') {
    ...
    if (!response.ok) {
      throw new ValidationError(
        `[gpu-device-api] GfxTexture.fromImage: fetching "${source}" failed with HTTP ${response.status}.`,
      );
    }
