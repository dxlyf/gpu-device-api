...
import type { Frustum } from '../utils/math/frustum.js';
...
export class FrustumCuller {
  ...
  private readonly planes: Frustum = frustumMath.create();
  private readonly worldCenter: Vec3 = vec3.create();
  private readonly scaling: Vec3 = vec3.create();
  ...
  intersectsLocalSphere(sphere: BoundingSphere, model: Mat4 | null | undefined): boolean {
    if (!model) return this.intersectsSphere(sphere.center, sphere.radius);

    mat4.transformPoint(this.worldCenter, model, sphere.center);
    if (sphere.radius <= 0) return this.intersectsSphere(this.worldCenter, 0);
