...
import { clamp, degToRad, vec3, type Vec3 } from '../utils/math/index.js';
...
interface PointerState {
  x: number;
  y: number;
  /** `pointerType`，用来区分鼠标与触摸。 */
  type: string;
  /** 按下时的按键编号，0 是左键、1 是中键、2 是右键。 */
  button: number;
}
...
export class OrbitControls {
  ...
  private scale = 1;
  /** 本帧待消耗的平移增量（世界空间）。 */
  private readonly panOffset: Vec3 = vec3.create();

  /** `reset()` 要恢复到的初始状态。 */
  private readonly initialPosition: Vec3 = vec3.create();
  private readonly initialTarget: Vec3 = vec3.create();
  private readonly previousPosition: Vec3 = vec3.create();
  private readonly previousTarget: Vec3 = vec3.create();
  ...
  update(): boolean {
    ...
    this.phi = clamp(
      this.phi,
      Math.max(this.minPolarAngle, POLAR_EPSILON),
      Math.min(this.maxPolarAngle, Math.PI - POLAR_EPSILON),
    );
    ...
    vec3.set(
      position,
      target[0]! + sinPhiRadius * Math.sin(this.theta),
      target[1]! + Math.cos(this.phi) * radius,
      target[2]! + sinPhiRadius * Math.cos(this.theta),
    );
  ...
  /** 所有活动指针是否都是触摸。 */
  private allPointersAreTouch(): boolean {
    for (const pointer of this.pointers.values()) {
      if (pointer.type !== 'touch') return false;
    }
    return true;
  }
