...
export function makeEmpty(out: Box3): Box3 {
...
export function isEmpty(a: Box3): boolean {
  return a.max[0]! < a.min[0]! || a.max[1]! < a.min[1]! || a.max[2]! < a.min[2]!;
}

export function clone(a: Box3): Box3 {
  return {
...
export function copy(out: Box3, a: Box3): Box3 {
  out.min[0] = a.min[0]!;
  out.min[1] = a.min[1]!;
  out.min[2] = a.min[2]!;
  out.max[0] = a.max[0]!;
...
export function set(out: Box3, minPoint: Vec3, maxPoint: Vec3): Box3 {
...
/** 由一组点求包围盒；空数组会得到空盒。 */
export function setFromPoints(out: Box3, points: readonly Vec3[]): Box3 {
  makeEmpty(out);
  for (const point of points) expandByPoint(out, point);
  return out;
}
 ...
 */
export function setFromArray(out: Box3, array: ArrayLike<number>, stride = 3): Box3 {
  makeEmpty(out);
...
export function getCenter(out: Vec3, a: Box3): Vec3 {
  if (isEmpty(a)) return out;
...
/** 尺寸（`max - min`）；空盒得到全 0。 */
export function getSize(out: Vec3, a: Box3): Vec3 {
  if (isEmpty(a)) {
    out[0] = 0;
    out[1] = 0;
    out[2] = 0;
...
export function getBoundingSphere(outCenter: Vec3, a: Box3): number {
  if (isEmpty(a)) {
    outCenter[0] = 0;
    outCenter[1] = 0;
    outCenter[2] = 0;
...
export function expandByPoint(out: Box3, point: Vec3): Box3 {
  min(out.min, out.min, point);
  max(out.max, out.max, point);
  return out;
}

export function expandByVector(out: Box3, vector: Vec3): Box3 {
  add(out.min, out.min, vector);
  add(out.max, out.max, vector);
  return out;
}

export function expandByScalar(out: Box3, scalar: number): Box3 {
...
export function containsPoint(a: Box3, point: Vec3): boolean {
  return (
    point[0]! >= a.min[0]! &&
    point[0]! <= a.max[0]! &&
    point[1]! >= a.min[1]! &&
...
export function containsBox(a: Box3, b: Box3): boolean {
  return (
    a.min[0]! <= b.min[0]! &&
    b.max[0]! <= a.max[0]! &&
    a.min[1]! <= b.min[1]! &&
...
export function intersectsSphere(a: Box3, center: Vec3, radius: number): boolean {
 ...
 */
export function intersectsPlane(a: Box3, plane: Plane): boolean {
  if (isEmpty(a)) return false;
...
export function clampPoint(out: Vec3, a: Box3, point: Vec3): Vec3 {
  if (isEmpty(a)) return out;
...
export function distanceToPoint(a: Box3, point: Vec3): number {
  if (isEmpty(a)) return 0;
  return Math.sqrt(squaredDistance(clampPoint(CORNER_SCRATCH, a, point), point));
}

export function translate(out: Box3, a: Box3, offset: Vec3): Box3 {
  copy(out, a);
...
export function union(out: Box3, a: Box3, b: Box3): Box3 {
  if (isEmpty(a)) return copy(out, b);
  if (isEmpty(b)) return copy(out, a);
...
/** 交集；不相交时得到空盒。 */
export function intersect(out: Box3, a: Box3, b: Box3): Box3 {
  if (!intersectsBox(a, b)) return makeEmpty(out);
...
export function applyMat4(out: Box3, a: Box3, m: Float32Array): Box3 {
  if (isEmpty(a)) return makeEmpty(out);
...
export function scaleBox(out: Box3, a: Box3, scalar: number): Box3 {
...
export function equals(a: Box3, b: Box3, epsilon = 1e-6): boolean {
...
/** 仅用于调试。 */
export function toString(a: Box3): string {
  if (isEmpty(a)) return 'box3(empty)';
