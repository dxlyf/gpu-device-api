...
export function clone(a: Ray): Ray {
  return {
...
export function copy(out: Ray, a: Ray): Ray {
...
export function set(out: Ray, origin: Vec3, direction: Vec3): Ray {
...
export function at(out: Vec3, a: Ray, t: number): Vec3 {
...
export function recast(out: Ray, a: Ray, t: number): Ray {
  const ox = a.origin[0]!;
  const oy = a.origin[1]!;
  const oz = a.origin[2]!;
  out.direction[0] = a.direction[0]!;
...
/** 点到射线上最近的点（`t < 0` 时夹到起点）。 */
export function closestPointToPoint(out: Vec3, a: Ray, point: Vec3): Vec3 {
  const t = Math.max(0, dot(sub(TEMP, point, a.origin), a.direction));
  return at(out, a, t);
}

/** 点到射线的距离（垂直距离，不是沿射线的参数）。 */
export function distanceToPoint(a: Ray, point: Vec3): number {
  return Math.sqrt(squaredDistanceToPoint(a, point));
}

/** 点到射线距离的平方。 */
export function squaredDistanceToPoint(a: Ray, point: Vec3): number {
 ...
 */
export function applyMat4(out: Ray, a: Ray, m: Float32Array): Ray {
 ...
 */
export function intersectPlane(a: Ray, plane: Plane): number | null {
...
export function intersectSphere(a: Ray, center: Vec3, radius: number): number | null {
 ...
 */
export function intersectBox(a: Ray, box: Box3): number | null {
...
export function intersectTriangle(
  ...
  if (backfaceCulling ? determinant < 1e-12 : Math.abs(determinant) < 1e-12) return null;
...
/** 起点与方向都接近才算相等。 */
export function equals(a: Ray, b: Ray, epsilon = 1e-6): boolean {
  return (
...
/** 仅用于调试。 */
export function toString(a: Ray): string {
...
export function isWellFormed(a: Ray): boolean {
  return (
    Number.isFinite(a.origin[0]!) &&
