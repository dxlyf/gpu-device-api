...
export function create(r = 0, g = 0, b = 0, a = 1): ColorValue {
  return { r, g, b, a };
}

export function clone(a: ColorValue): ColorValue {
  return { r: a.r, g: a.g, b: a.b, a: a.a };
}

export function copy(out: ColorValue, a: ColorValue): ColorValue {
  out.r = a.r;
  out.g = a.g;
  out.b = a.b;
  out.a = a.a;
...
export function set(out: ColorValue, r: number, g: number, b: number, a = 1): ColorValue {
  out.r = r;
  out.g = g;
  out.b = b;
  out.a = a;
...
export function setRGB(out: ColorValue, r: number, g: number, b: number): ColorValue {
  out.r = r;
  out.g = g;
  out.b = b;
  return out;
...
export function setHex(out: ColorValue, hex: number, alpha?: number): ColorValue {
...
export function getHex(a: ColorValue): number {
 ...
 */
export function setStyle(out: ColorValue, style: string): ColorValue {
  ...
  if (text.startsWith('#')) {
    ...
    if (/^[0-9a-f]+$/.test(value)) {
      const expand = (part: string): number => parseInt(part + part, 16) / 255;
      if (value.length === 3 || value.length === 4) {
        out.r = expand(value[0]!);
        out.g = expand(value[1]!);
        out.b = expand(value[2]!);
        if (value.length === 4) out.a = expand(value[3]!);
        return out;
      }
      if (value.length === 6 || value.length === 8) {
        const channel = (index: number): number => parseInt(value.slice(index, index + 2), 16) / 255;
        out.r = channel(0);
        out.g = channel(2);
        out.b = channel(4);
        if (value.length === 8) out.a = channel(6);
        return out;
      }
    }
  ...
  if (functional) {
    ...
    if (parts.length >= 3 && parts.slice(0, 3).every((part) => Number.isFinite(part))) {
      ...
      out.g = parts[1]! / 255;
      out.b = parts[2]! / 255;
      if (parts.length >= 4 && Number.isFinite(parts[3]!)) out.a = parts[3]!;
      return out;
    }
  ...
  throw new RangeError(
    `[gpu-device-api] color.setStyle: unknown color "${style}". Supported: #rgb / #rrggbb / #rrggbbaa, ` +
      'rgb() / rgba(), and a small set of CSS color names.',
  );
}

/** 转成 `#rrggbb`（或带 alpha 时的 `#rrggbbaa`）字符串。 */
export function getStyle(a: ColorValue, includeAlpha = false): string {
  const toByte = (value: number): number => Math.round(Math.min(Math.max(value, 0), 1) * 255);
...
/** 逐分量夹到 `[0, 1]`。 */
export function clampColor(out: ColorValue, a: ColorValue): ColorValue {
...
/** 线性插值（`t` 不夹紧）。 */
export function lerp(out: ColorValue, a: ColorValue, b: ColorValue, t: number): ColorValue {
  out.r = a.r + (b.r - a.r) * t;
  out.g = a.g + (b.g - a.g) * t;
  out.b = a.b + (b.b - a.b) * t;
  out.a = a.a + (b.a - a.a) * t;
...
export function add(out: ColorValue, a: ColorValue, b: ColorValue): ColorValue {
  out.r = a.r + b.r;
  out.g = a.g + b.g;
  out.b = a.b + b.b;
  out.a = a.a + b.a;
...
export function multiply(out: ColorValue, a: ColorValue, b: ColorValue): ColorValue {
  out.r = a.r * b.r;
  out.g = a.g * b.g;
  out.b = a.b * b.b;
  out.a = a.a * b.a;
...
export function setHSL(out: ColorValue, h: number, s: number, l: number, a = out.a): ColorValue {
...
export function getHSL(out: Float32Array, a: ColorValue): Float32Array {
  ...
  let hue: number;
  if (maxChannel === a.r) hue = (a.g - a.b) / delta + (a.g < a.b ? 6 : 0);
  else if (maxChannel === a.g) hue = (a.b - a.r) / delta + 2;
  else hue = (a.r - a.g) / delta + 4;
...
export function toArray(a: ColorValue, out?: Float32Array, includeAlpha = true): Float32Array {
  const target = out ?? new Float32Array(includeAlpha ? 4 : 3);
  target[0] = a.r;
  target[1] = a.g;
  target[2] = a.b;
  if (includeAlpha && target.length >= 4) target[3] = a.a;
  return target;
}

/** 从长度为 3 或 4 的扁平数组读取。 */
export function fromArray(out: ColorValue, array: ArrayLike<number>, offset = 0): ColorValue {
  out.r = array[offset] ?? 0;
  out.g = array[offset + 1] ?? 0;
  out.b = array[offset + 2] ?? 0;
  if (array.length > offset + 3) out.a = array[offset + 3]!;
  return out;
}
 ...
 */
export function convertSRGBToLinear(out: ColorValue, a: ColorValue): ColorValue {
  const channel = (value: number): number =>
    value < 0.04045 ? value * 0.0773993808 : Math.pow(value * 0.9478672986 + 0.0521327014, 2.4);
  out.r = channel(a.r);
  out.g = channel(a.g);
  out.b = channel(a.b);
  out.a = a.a;
...
export function convertLinearToSRGB(out: ColorValue, a: ColorValue): ColorValue {
  const channel = (value: number): number =>
    value <= 0.0031308 ? value * 12.92 : 1.055 * Math.pow(value, 0.41666) - 0.055;
  out.r = channel(a.r);
  out.g = channel(a.g);
  out.b = channel(a.b);
  out.a = a.a;
...
export function isInGamut(a: ColorValue, epsilon = 1e-6): boolean {
  return (
    a.r >= -epsilon &&
    a.r <= 1 + epsilon &&
    a.g >= -epsilon &&
...
export function equals(a: ColorValue, b: ColorValue, epsilon = 1e-6): boolean {
  return (
...
export function toString(a: ColorValue): string {
