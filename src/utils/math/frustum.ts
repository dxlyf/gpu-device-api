...
import {
  copy as copyPlane,
  create as createPlane,
  distanceToPoint as planeDistanceToPoint,
  equals as planeEquals,
  normalize as normalizePlane,
  setComponents as setPlaneComponents,
} from './plane.js';
...
export function clone(a: Frustum): Frustum {
  const out = create();
  return copy(out, a);
}

export function copy(out: Frustum, a: Frustum): Frustum {
  for (let i = 0; i < 6; i++) copyPlane(out.planes[i]!, a.planes[i]!);
  return out;
}
 ...
 */
export function setFromProjectionView(out: Frustum, m: Float32Array, depthRange: 'gl' | 'zo' = 'gl'): Frustum {
  ...
  const combined = (a: readonly number[], b: readonly number[], sign: number): [number, number, number, number] => [
    a[0]! + sign * b[0]!,
    a[1]! + sign * b[1]!,
    a[2]! + sign * b[2]!,
    a[3]! + sign * b[3]!,
...
/** 点是否在视锥体内（6 个面都在内侧）。 */
export function containsPoint(a: Frustum, point: Vec3): boolean {
  for (const frustumPlane of a.planes) {
    if (planeDistanceToPoint(frustumPlane, point) < 0) return false;
  }
  return true;
}

/** 球是否与视锥体相交（可能被看到）。 */
export function intersectsSphere(a: Frustum, center: Vec3, radius: number): boolean {
  for (const frustumPlane of a.planes) {
    if (planeDistanceToPoint(frustumPlane, center) < -radius) return false;
  }
  return true;
}
 ...
 */
export function intersectsBox(a: Frustum, box: Box3): boolean {
  for (const frustumPlane of a.planes) {
    ...
    if (nx * x + ny * y + nz * z + frustumPlane.constant < 0) return false;
  }
  return true;
}
 ...
 */
export function intersectsPlane(a: Frustum, other: Plane): boolean {
  ...
  for (const corner of corners) {
    ...
    if (distance > 0) positive = true;
    else if (distance < 0) negative = true;
 ...
 */
function frustumCorners(a: Frustum): Vec3[] | null {
  const [left, right, bottom, top, near, far] = a.planes as [Plane, Plane, Plane, Plane, Plane, Plane];
  const corners: Vec3[] = [];
...
export function equals(a: Frustum, b: Frustum, epsilon = 1e-6): boolean {
  for (let i = 0; i < 6; i++) {
    if (!planeEquals(a.planes[i]!, b.planes[i]!, epsilon)) return false;
  }
  return true;
}
