...
export const DEFAULT_TEXTURE_SWIZZLE: TextureSwizzleString = 'rgba';
 ...
 */
export function assertTextureSwizzle(
  swizzle: unknown,
  context: string,
): asserts swizzle is TextureSwizzleString {
  if (typeof swizzle !== 'string' || !/^[rgba01]{4}$/.test(swizzle)) {
    throw new ValidationError(
      `[gpu-device-api] ${context}: swizzle must be a four-character string made of "r", "g", "b", "a", ` +
