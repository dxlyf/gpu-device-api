 ...
 */
export function mipLevelExtent(
  size: TextureSize,
  level: number,
  ...
  if (
    ...
    throw new ValidationError(
      `[gpu-device-api] mipLevelExtent: dimension must be "1d", "2d" or "3d", got ${String(dimension)}.`,
    );
  }
  const extent = resolveTextureSize(size);
  ...
  return {
    ...
    depthOrArrayLayers:
      dimension === TextureDimension.D3 ? shift(extent.depthOrArrayLayers) : extent.depthOrArrayLayers,
  };
}
