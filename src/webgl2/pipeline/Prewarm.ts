 ...
 */
export function programCacheOf(device: Device): ProgramCache {
  const candidate = device as unknown as { programs?: unknown };
  if (candidate.programs instanceof ProgramCache) return candidate.programs;
  throw new ValidationError(
    '[gpu-device-api] prewarmWebGL2RenderPipeline: expected a WebGL2 device created by this library ' +
 ...
 */
export async function prewarmWebGL2RenderPipeline(
  device: Device,
  ...
  if (!descriptor.fragment) {
    ...
    throw new ValidationError(
      '[gpu-device-api] prewarmWebGL2RenderPipeline: WebGL2 needs both a vertex and a fragment stage ' +
  ...
  if (!result.ok) {
    if (options.throwOnError) throw new ValidationError(describeCompilationInfo(result.info));
