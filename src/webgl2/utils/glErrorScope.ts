 ...
 */
export function formatGlErrorCode(code: number): string {
 ...
 * 其余一律当 `'validation'`。
 */
export function classifyGlError(error: GpuError): ErrorScopeFilter {
  if (error instanceof OutOfMemoryError) return 'out-of-memory';
 ...
 */
export function createGlError(code: number, context: string): GpuError {
  const hex = formatGlErrorCode(code);
  const details: Record<string, unknown> = { glError: code, context };
  if (code === GL_ERROR_CODES.OUT_OF_MEMORY) {
    return new OutOfMemoryError(
      `[gpu-device-api] GL out of memory (${hex}) observed during ${context}.`,
      { details },
    );
  }
  if (
    code === GL_ERROR_CODES.INVALID_ENUM ||
    code === GL_ERROR_CODES.INVALID_VALUE ||
    code === GL_ERROR_CODES.INVALID_OPERATION ||
    code === GL_ERROR_CODES.INVALID_FRAMEBUFFER_OPERATION
  ) {
    return new ValidationError(
      `[gpu-device-api] GL error ${hex} observed during ${context}. This code means "the API was used ` +
        'incorrectly", but GL does not tell which call produced it, nor whether the real cause was ' +
        'validation, memory or the implementation itself.',
      { details },
    );
  }
  if (code === GL_ERROR_CODES.CONTEXT_LOST_WEBGL) {
    return new GPUInternalError(
      `[gpu-device-api] GL context lost (${hex}) observed during ${context}. Every GL object created by ` +
        'this device is now invalid; recreate the device and its resources.',
      { details },
    );
  }
  return new GPUInternalError(
    `[gpu-device-api] unknown GL error ${hex} observed during ${context}. GL does not define this code, ` +
      'so it comes from the driver or a vendor extension and cannot be classified.',
    { details },
  );
}
 ...
 */
export class WebGL2ErrorScope implements ErrorScopeHandle {
  readonly filter: ErrorScopeFilter;
  readonly label: string;

  private readonly errorsSeen: GpuError[] = [];
  private readonly settle: (scope: WebGL2ErrorScope) => GpuError | null;
  ...
  constructor(
    ...
    label: string,
    settle: (scope: WebGL2ErrorScope) => GpuError | null,
  ) {
    this.filter = filter;
    this.label = label;
    this.settle = settle;
  ...
  pop(): Promise<GpuError | null> {
      ...
      return Promise.reject(
        new ValidationError(
          `[gpu-device-api] error scope "${this.label}" can no longer be settled: the device was disposed ` +
            'while this scope was still on the stack, so the GL error queue cannot be read any more. ' +
    ...
    // 同步排空 + 归属判断由宿主设备完成（只有它能看见整个栈），结果包成已落定的 promise。
    const result = this.settle(this);
