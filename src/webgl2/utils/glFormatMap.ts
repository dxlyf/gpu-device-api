...
export const GL_TEXTURE_FORMATS: Readonly<Record<string, GlTextureFormatInfo>> = Object.freeze({
...
const UNSUPPORTED: Readonly<Record<string, string>> = Object.freeze({
  bgra8unorm:
    'WebGL2 没有 bgra8unorm 纹理格式（BGRA 只是默认帧缓冲的隐含排布）。请改用 rgba8unorm。',
  'bgra8unorm-srgb':
    'WebGL2 没有 bgra8unorm-srgb 纹理格式。请改用 rgba8unorm-srgb。',
  rgb9e5ufloat:
    'WebGL2 不支持 rgb9e5ufloat（无法作为纹理存储格式，也不能从主机上传）。请改用 rg11b10ufloat 或 rgba16float。',
  stencil8:
    'WebGL2 的 STENCIL_INDEX8 只能用作 renderbuffer，不能作为纹理格式。请改用 depth24plus-stencil8。',
});

/** 取出某个格式在 WebGL2 下的映射；不支持时抛出带替代方案的错误。 */
export function glFormat(format: TextureFormat): GlTextureFormatInfo {
...
export function glFormatIsAttachment(format: TextureFormat): boolean {
  return glFormat(format).attachment;
}

/** 采样器类型是否与纹理格式匹配（整数纹理必须配 `isampler2D` / `usampler2D`）。 */
export function sampleTypeMatchesFormat(
  format: TextureFormat,
  sampleType: 'float' | 'unfilterable-float' | 'depth' | 'sint' | 'uint',
): boolean {
  const info = glFormat(format);
  if (info.sampleType === 'depth') return sampleType === 'depth' || sampleType === 'unfilterable-float';
  if (info.sampleType === 'uint') return sampleType === 'uint';
  if (info.sampleType === 'sint') return sampleType === 'sint';
 ...
 */
export function assertUploadDataType(format: TextureFormat, data: ArrayBufferView): void {
  const info = glFormat(format);
  if (info.uploadType === null) {
    throw new ValidationError(
      `[gpu-device-api] 纹理格式「${format}」不支持从主机内存上传。`,
    );
  }
  const actual = data.constructor.name;
  if (actual !== info.uploadType) {
    const hint =
      format === 'rgba16float' || format === 'r16float' || format === 'rg16float'
        ? '（该格式是 half float，需要先把 Float32 转成 Uint16 位模式，可用 Float32Array 与 Uint16Array 共享同一段内存来做转换。）'
        : '';
    throw new ValidationError(
      `[gpu-device-api] 纹理格式「${format}」要求主机数据是 ${info.uploadType}，实际传入 ${actual}。${hint}`,
    );
  }
}
