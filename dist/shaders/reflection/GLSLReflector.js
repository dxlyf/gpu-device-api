...
export function isSamplerType(glType) {
    return GLSL_SAMPLER_TYPES.includes(glType);
}
/** 查询一个已链接 program 的全部接口信息。 */
export function reflectGlslProgram(gl, program) {
    const attributes = [];
    ...
    for (let i = 0; i < blockCount; i++) {
        ...
        uniformBlocks.push({
            ...
            activeUniforms: gl.getActiveUniformBlockParameter(program, i, gl.UNIFORM_BLOCK_ACTIVE_UNIFORMS),
        });
    }
    return { attributes, uniforms, uniformBlocks };
}
/** 把采样器 uniform 名字列出来，供纹理单元分配使用。 */
export function reflectSamplerUniforms(reflection) {
 ...
 */
export function inferBindGroupLayoutEntries(reflection, visibility) {
    const entries = [];
...
//# sourceMappingURL=GLSLReflector.js.map