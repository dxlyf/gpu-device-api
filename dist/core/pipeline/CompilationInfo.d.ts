...
export declare function createCompilationInfo(input: {
    label: string;
    backend: BackendKind;
...
export declare function mergeCompilationInfo(label: string, backend: BackendKind, infos: readonly CompilationInfo[]): CompilationInfo;
/**
 * 一份「没有诊断」的结果。
 *
 ...
 * 而不是假装「编译干净」。
 */
export declare function emptyCompilationInfo(label: string, backend: BackendKind): CompilationInfo;
/** 构造一条诊断，顺手补上 label / backend 这些公共字段。 */
export declare function createCompilationMessage(input: {
...
export declare function compilationStageName(stage: ShaderStage | null): string;
/**
 ...
 */
export declare function formatCompilationMessage(message: CompilationMessage): string;
/** 把一整份 {@link CompilationInfo} 输出成多行文本（每条 message 一行）。 */
export declare function describeCompilationInfo(info: CompilationInfo): string;
/** 运行时判断：这个值是不是一份 {@link CompilationInfo}（用于跨包边界的收窄）。 */
export declare function isCompilationInfo(value: unknown): value is CompilationInfo;
/**
 ...
 */
export declare function parseGlCompilationLog(log: string, label: string, backend: BackendKind, stage: ShaderStage | null): CompilationMessage[];
/**
 * 预热是怎么等出来的。
 *
...
export declare const DEFAULT_PREWARM_TIMEOUT_MS = 30000;
...
//# sourceMappingURL=CompilationInfo.d.ts.map