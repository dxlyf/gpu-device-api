...
export function validateVertexBufferLayout(layout, limits) {
    ...
    if (layout.arrayStride > limits.maxVertexBufferArrayStride) {
        throw new Error(`[gpu-device-api] VertexBufferLayout.arrayStride ${layout.arrayStride} exceeds maxVertexBufferArrayStride (${limits.maxVertexBufferArrayStride}).`);
    }
    ...
    for (const attribute of layout.attributes) {
        ...
        if (attribute.shaderLocation < 0 || attribute.shaderLocation >= limits.maxVertexAttributes) {
            throw new Error(`[gpu-device-api] VertexAttribute.shaderLocation ${attribute.shaderLocation} is outside [0, ${limits.maxVertexAttributes - 1}].`);
        }
        ...
        if (attribute.offset + info.byteSize > layout.arrayStride) {
            throw new Error(`[gpu-device-api] VertexAttribute at location ${attribute.shaderLocation} reads ${info.byteSize} bytes from offset ${attribute.offset}, ` +
 ...
 */
const layoutsKeyCache = new WeakMap();
export function vertexBufferLayoutsKey(layouts) {
