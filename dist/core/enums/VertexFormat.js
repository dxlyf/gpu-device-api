function info(components, byteSize, kind, normalized = false) {
    return { components, byteSize, kind, normalized };
}
/** 所有 {@link VertexFormat} 的共享描述，与后端无关。 */
export const VERTEX_FORMAT_INFO = Object.freeze({
...
export function vertexFormatInfo(format) {
    ...
    if (!value)
        throw new Error(`[gpu-device-api] Unknown vertex format "${format}".`);
    return value;
}
/** 用于声明该格式 attribute 的 GLSL ES 3.00 类型。 */
export function vertexFormatGlslType(format) {
    const value = vertexFormatInfo(format);
    const table = value.kind === 'float'
...
export function vertexFormatWgslType(format) {
    const value = vertexFormatInfo(format);
    const table = value.kind === 'float'
...
//# sourceMappingURL=VertexFormat.js.map