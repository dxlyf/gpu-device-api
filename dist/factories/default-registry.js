...
class WebGPUFactory {
    ...
    async isAvailable(options) {
        ...
        try {
            ...
            return { ok: false, reason: `requestAdapter() 抛错：${error.message}` };
        }
    }
    async createAdapter(options) {
        ...
        const created = await withTimeout(WebGPUAdapter.create({
            powerPreference: options.powerPreference,
            forceFallbackAdapter: options.forceFallbackAdapter,
        }), WEBGPU_ADAPTER_PROBE_TIMEOUT_MS);
        if (created === TIMED_OUT) {
            throw new ValidationError(`[gpu-device-api] WebGPU 的 requestAdapter() 超过 ${WEBGPU_ADAPTER_PROBE_TIMEOUT_MS}ms 没有返回。` +
...
class WebGL2Factory {
    ...
    async isAvailable(options) {
        ...
        try {
            ...
            return { ok: false, reason: `创建 WebGL2 context 时抛错：${error.message}` };
        }
    }
    async createAdapter(options) {
