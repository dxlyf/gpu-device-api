...
export function isSampledTextureEntry(entry) {
    return entry.type === 'texture';
}
/** 布局里是否包含任何 WebGL2 无法表达的东西（storage buffer / storage texture）。 */
export function assertLayoutSupportedByWebGL2(layout) {
    for (const entry of layout.sortedEntries) {
        if (entry.type === 'storage' || entry.type === 'read-only-storage') {
            throw new ValidationError(`[gpu-device-api] BindGroupLayout「${layout.label}」的 binding ${entry.binding} 是 storage buffer，` +
                'WebGL2 不支持。请改用 uniform buffer。');
        }
        if (entry.type === 'storage-texture') {
            throw new ValidationError(`[gpu-device-api] BindGroupLayout「${layout.label}」的 binding ${entry.binding} 是 storage texture，` +
                'WebGL2 不支持。请改用「渲染到纹理 + 采样」。');
        }
    }
}
//# sourceMappingURL=WebGL2BindGroupLayout.js.map