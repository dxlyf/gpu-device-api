...
import { GL_ERROR_CODES, WebGL2ErrorScope, classifyGlError, createGlError, } from './utils/glErrorScope.js';
...
export class WebGL2Device {
    ...
    contextRestoredCallbacks = new Set();
    ...
    constructor(options) {
        ...
        if (missing.length > 0) {
            throw new ValidationError(`[gpu-device-api] WebGL2 适配器不支持以下必需特性：${missing.join('、')}。\n` +
                `当前可用特性：${featureNames.join('、') || '(无)'}。`);
        }
        this.features = {
            has: (feature) => featureSet.has(feature),
            names: featureNames,
        };
    ...
    createBuffer(descriptor) {
        ...
        return this.track(new WebGL2Buffer(this.gl, this.state, descriptor, (buffer) => {
    ...
    createTexture(descriptor) {
        ...
        return this.track(new WebGL2Texture(this.gl, this.state, descriptor, (texture) => {
    ...
    createAttachmentTexture(format, width, height, usage, label) {
        const texture = new WebGL2Texture(this.gl, this.state, { format, size: { width, height }, usage: usage, label }, (destroyed) => {
    ...
    createSampler(descriptor = {}) {
        ...
        const sampler = new WebGL2Sampler(this.gl, this.state, descriptor, (destroyed) => this.untrack(destroyed));
    ...
    importExternalTexture(descriptor) {
        ...
        if (descriptor === undefined || descriptor.source === undefined || descriptor.source === null) {
            throw new ValidationError('[gpu-device-api] Device.importExternalTexture: "source" is required (an HTMLVideoElement, ' +
                'VideoFrame or ImageBitmap).');
        }
        throw new ValidationError('[gpu-device-api] Device.importExternalTexture is not supported by the WebGL2 backend: GL has no ' +
    ...
    readQuerySet(querySet, options = {}) {
        ...
        if (!Number.isInteger(first) || first < 0) {
            throw new ValidationError(`[gpu-device-api] Device "${this.label}".readQuerySet: firstQuery must be a non-negative integer, got ` +
                `${String(first)}.`);
        }
    ...
    createRenderPipeline(descriptor) {
        ...
        if (!descriptor.fragment) {
            throw new ValidationError('[gpu-device-api] WebGL2 后端要求管线同时提供顶点与片元着色器（GL 的 program 必须链接两个阶段）。\n' +
        ...
        if (descriptor.layout === undefined || descriptor.layout === 'auto') {
            ...
            if (entries.length === 0) {
                ...
                const synthesizedLayout = new WebGL2PipelineLayout({ label: `${label}:autoPipelineLayout`, bindGroupLayouts: [inferredLayout] }, true, this, () => this.untrack(synthesizedLayout));
                layout = this.track(synthesizedLayout);
                this.programs.bindPlan(compiled, layout.bindingPlan);
            }
        }
        else {
            /*
             ...
             */
            const resolved = resolvePipelineLayoutLike(descriptor.layout, (bindGroupLayouts) => {
                const synthesized = new WebGL2PipelineLayout({ label: `${label}:inlinePipelineLayout`, bindGroupLayouts }, false, this, () => this.untrack(synthesized));
            ...
            if (resolved.synthesized)
                synthesizedLayouts.push(resolved.synthesized);
        }
        const pipeline = new WebGL2RenderPipeline(descriptor, compiled, layout, {
            ...
            onDispose: (destroyed) => {
                this.untrack(destroyed);
                for (const synthesized of synthesizedLayouts)
                    synthesized.dispose();
    ...
    createRenderTarget(descriptor = {}) {
        ...
        const target = new WebGL2RenderTarget(descriptor, {
            gl: this.gl,
            state: this.state,
            createTexture: (format, width, height, usage, label) => this.createAttachmentTexture(format, width, height, usage, label),
    ...
    createCanvasContext(canvas, config) {
        ...
        if (canvas !== this.canvas) {
            throw new ValidationError('[gpu-device-api] WebGL2 的 device 只能服务创建它的那个 canvas。\n' +
    ...
    pushErrorScope(filter) {
         ...
         */
        if (this.debug || this.scopeStack.length > 0)
            this.drainGlErrors('pushErrorScope');
        const scope = new WebGL2ErrorScope(filter, `webgl2ErrorScope#${nextId('scope')}`, (popped) => this.settleScope(popped));
     ...
     */
    drainGlErrors(observedBy) {
        const debug = this.debug;
        for (;;) {
            this.glErrorReads += 1;
            const code = this.gl.getError();
            if (code === GL_ERROR_CODES.NO_ERROR)
                return;
            const scope = this.scopeStack[this.scopeStack.length - 1];
            if (scope)
                scope.record(createGlError(code, observedBy));
            /*
             ...
             */
            if (debug)
                this.reportError(createLegacyGlError(code, observedBy));
        }
    }
    /**
    ...
    settleScope(scope) {
        ...
        if (position >= 0)
            this.scopeStack.splice(position, 1);
    ...
    dispose() {
        ...
        for (const context of this.canvasContexts.values())
            context.dispose();
        ...
        const destroyed = { reason: 'destroyed', message: 'Device.dispose() was called.' };
    ...
    assertUsable(operation) {
        ...
        if (lost) {
            throw new DeviceLostError(`[gpu-device-api] Device.${operation}: device "${this.label}" lost its WebGL2 context ` +
                `(${lost.reason}): ${lost.message}`, { reason: lost.reason });
        }
    }
    /**
    ...
    handleContextLostEvent() {
        ...
        const info = {
            reason: 'unknown',
            message: 'the WebGL2 context was lost (webglcontextlost). Every GL object created by this device is now ' +
        ...
        if (!this._disposed) {
            this.reportError(new DeviceLostError(`[gpu-device-api] WebGL2 context lost: ${info.message}`, { reason: info.reason }));
        }
    }
    /**
    ...
    handleContextRestoredEvent() {
        ...
        this.logger.warn(`WebGL2 context restored (第 ${this.contextRestoreCount} 次)：canvas 又能用了，但本设备创建过的` +
 ...
 * 只描述现象。
 */
function createLegacyGlError(code, context) {
...
export function describeGlAdapter(gl) {
 ...
 */
function readTimingSupport(gl) {
    // 假的 GL（测试桩）可能连 getExtension 都没有：拿不到扩展一律按「没有」处理。
    const extension = typeof gl.getExtension === 'function' ? gl.getExtension(TIMER_QUERY_EXTENSION) : null;
