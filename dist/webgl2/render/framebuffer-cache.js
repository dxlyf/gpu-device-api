 ...
 */
function viewSignature(attachment) {
...
export class FramebufferCache {
    ...
    constructor(gl, options = {}) {
        ...
        this.framebuffers = createPipelineCache(options.limit ?? 64, (framebuffer, key) => {
    ...
    acquire(descriptor) {
        ...
        if (slots > limit) {
            throw new ValidationError(`[gpu-device-api] 这次渲染通道有 ${slots} 个颜色附件槽位（含 null 空位），超过了本设备的 ` +
        ...
        for (let index = 0; index < slots; index += 1) {
            ...
            if (target === gl.TEXTURE_2D) {
                ...
                gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + index, target, view.glTexture, view.descriptor.baseMipLevel);
            }
            else {
                gl.framebufferTextureLayer(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + index, view.glTexture, view.descriptor.baseMipLevel, view.descriptor.baseArrayLayer);
            }
        ...
        if (depthAttachment) {
            const view = depthAttachment.view;
            ...
            referenced.push(view.texture);
            gl.framebufferTexture2D(gl.FRAMEBUFFER, attachmentPoint, gl.TEXTURE_2D, view.glTexture, view.descriptor.baseMipLevel);
        }
        ...
        if (status !== gl.FRAMEBUFFER_COMPLETE) {
            gl.deleteFramebuffer(framebuffer);
            throw new ValidationError(`[gpu-device-api] 这组渲染附件在 WebGL2 下不构成完整的 framebuffer（GL 状态码 0x${status.toString(16)}）。\n` +
    ...
    releaseTexture(texture) {
        ...
        for (const [key, referenced] of this.references) {
            if (referenced.includes(texture))
                doomed.push(key);
        }
...
//# sourceMappingURL=framebuffer-cache.js.map