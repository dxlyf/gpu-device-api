...
export class WebGL2Buffer {
    ...
    constructor(gl, state, descriptor, onDestroy) {
        ...
        if (descriptor.size % 4 !== 0) {
            throw new ValidationError(`[gpu-device-api] BufferDescriptor.size 必须是 4 的倍数，实际是 ${descriptor.size}。` +
                '（WebGPU 也有同样的限制，这里提前拦下以免两个后端行为不一致。）');
        }
        for (const unsupported of UNSUPPORTED_USAGE) {
            if ((descriptor.usage & unsupported.flag) !== 0) {
                throw new ValidationError(`[gpu-device-api] BufferUsage.${unsupported.name} 在 WebGL2 后端不可用：${unsupported.reason}`);
            }
        }
        this.gl = gl;
        this.state = state;
        ...
        if (!buffer)
            throw new ValidationError('[gpu-device-api] gl.createBuffer() 返回 null，无法分配 buffer。');
        this.native = buffer;
    ...
    async mapAsync(mode, offset = 0, size = this.size - offset) {
        this.assertUsable('mapAsync');
        if (this._mapState !== 'unmapped') {
            throw new ValidationError(`[gpu-device-api] buffer「${this.label}」已处于映射状态（mapState: "${this._mapState}"），请先 unmap()。`);
        }
        assertNonNegativeInteger(offset, 'mapAsync 的 offset');
        assertPositiveInteger(size, 'mapAsync 的 size');
        if (offset % 4 !== 0) {
            throw new ValidationError(`[gpu-device-api] mapAsync 的 offset 必须是 4 的倍数，实际是 ${offset}。（WebGPU 要求 8 的倍数，这里按更宽松的 4 处理。）`);
        }
        if (offset + size > this.size) {
            throw new ValidationError(`[gpu-device-api] mapAsync 的范围 [${offset}, ${offset + size}) 超出了 buffer 大小 ${this.size}。`);
        }
        if (mode === 'read') {
    ...
    getMappedRange(offset = 0, size) {
        ...
        if (this._mapState !== 'mapped' || !mapping) {
            throw new ValidationError(`[gpu-device-api] buffer「${this.label}」尚未映射（mapState: "${this._mapState}"），` +
        ...
        if (offset < 0 || length <= 0 || offset + length > mapping.size) {
            throw new ValidationError(`[gpu-device-api] getMappedRange(${offset}, ${length}) 超出已映射范围 ${mapping.size}。`);
        }
        if (offset === 0 && length === mapping.size)
            return mapping.data;
    ...
    unmap() {
        ...
        if (!mapping) {
            ...
            if (stale !== 'unmapped') {
                ...
                logger.warn(`buffer「${this.label}」的 mapState 是 "${stale}" 但内部没有映射记录；状态已复位为 'unmapped'。`);
            }
            return;
        }
        this.mapping = null;
    ...
    destroy() {
        ...
        if (this.mapping !== null && this.mapping.mode === 'write' && this.mapping.dirty) {
            logger.warn(`buffer「${this.label}」在被映射（mapState: "${this._mapState}"）的状态下被 destroy()，` +
    ...
    assertUsable(operation) {
        if (this._disposed) {
            throw new ValidationError(`[gpu-device-api] buffer「${this.label}」已销毁，不能再调用 ${operation}()。` +
                '这种情况通常是资源在 device.dispose() 之后仍被使用。');
        }
    }
}
//# sourceMappingURL=WebGL2Buffer.js.map