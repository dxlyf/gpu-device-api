 ...
 */
export function resolveGlMinFilter(minFilter, mipmapFilter, mipLevelCount) {
    if (mipLevelCount <= 1) {
...
export class WebGL2Sampler {
    ...
    mipLevelCount = MIPS_ASSUMED;
    constructor(gl, state, descriptor = {}, onDispose = () => { }) {
        this.gl = gl;
        this.state = state;
        ...
        if (!sampler)
            throw new ValidationError('[gpu-device-api] gl.createSampler() 返回 null，无法分配 sampler。');
        this.native = sampler;
        ...
        if (resolved.magFilter !== 'nearest' && resolved.magFilter !== 'linear') {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": magFilter must be "nearest" or "linear" ` +
        ...
        if (resolved.minFilter !== 'nearest' && resolved.minFilter !== 'linear') {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": minFilter must be "nearest" or "linear", ` +
                `got "${String(resolved.minFilter)}".`);
        }
        if (resolved.mipmapFilter !== 'nearest' && resolved.mipmapFilter !== 'linear') {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": mipmapFilter must be "nearest" or "linear", ` +
        ...
        if (resolved.maxAnisotropy > 1) {
            ...
            if (extension) {
                ...
                gl.samplerParameterf(sampler, extension.TEXTURE_MAX_ANISOTROPY_EXT, Math.min(resolved.maxAnisotropy, max));
            }
     ...
     */
    get minFilter() {
        return resolveGlMinFilter(this.descriptor.minFilter, this.descriptor.mipmapFilter, this.mipLevelCount);
    }
...
//# sourceMappingURL=WebGL2Sampler.js.map