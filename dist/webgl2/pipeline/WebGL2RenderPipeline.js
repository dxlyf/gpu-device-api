...
import { applyRenderState, resolveRenderState } from './WebGL2RenderState.js';
/**
 * 每个管线变体缓存的 VAO 上限。
 *
...
export class WebGL2RenderPipeline {
    ...
    _disposed = false;
    constructor(descriptor, program, layout, options) {
        if (!descriptor.fragment) {
            throw new ValidationError('[gpu-device-api] WebGL2 后端不支持只有深度、没有片元着色器的管线（GL 的 program 必须同时链接两个阶段）。\n' +
        ...
        if (!Number.isInteger(cacheLimit) || cacheLimit < 1) {
            throw new ValidationError(`[gpu-device-api] vertexArrayCacheLimit 必须是 >= 1 的整数，实际是 ${String(cacheLimit)}。`);
        }
        ...
        this.plan = layout === 'auto' ? null : (layout.bindingPlan ?? null);
    ...
    resolveVariant(variant = {}) {
        ...
        if (vertexLayouts.length > 0) {
            for (const layout of vertexLayouts)
                validateVertexBufferLayout(layout, this.limits);
        }
         ...
         */
        const programLocations = new Set(this.program.reflection.attributes
        ...
        for (const location of programLocations) {
            if (!declaredLocations.has(location)) {
                throw new ValidationError(`[gpu-device-api] 管线「${this.label}」的顶点着色器声明了 location ${location}，` +
        ...
        const resolved = {
             ...
             */
            vertexArrays: createPipelineCache(this.vertexArrayCacheLimit, (vertexArray) => {
     ...
     */
    evictVertexArray(variant, vertexArray) {
        const lookup = variant.vertexArrayLookup;
        if (lookup !== null && lookup.vertexArray === vertexArray)
            variant.vertexArrayLookup = null;
        if (this.state.currentVertexArray === vertexArray)
     ...
     */
    async prewarm(_variant = {}, _options = {}) {
        const started = nowMs();
        if (this._disposed) {
     ...
     */
    applyState(variant, stencilReference = 0) {
        ...
        if (this.alphaToCoverage) {
            if (this.declaredSampleCount <= 1 && variant.sampleCount <= 1) {
                throw new ValidationError(`[gpu-device-api] RenderPipeline "${this.label}": MultisampleState.alphaToCoverageEnabled ` +
     ...
     */
    acquireVertexArray(variant, bindings, indexBuffer, bindingRevision = 0) {
        ...
        if (cached) {
            if (bindingRevision > 0)
                variant.vertexArrayLookup = { revision: bindingRevision, vertexArray: cached };
            return cached;
        }
        const gl = this.gl;
        ...
        for (let slot = 0; slot < layouts.length; slot++) {
            ...
            for (const attribute of layout.attributes) {
                ...
                if (info.integer) {
                    ...
                    gl.vertexAttribPointer(attribute.shaderLocation, info.size, info.type, info.normalized, layout.arrayStride, offset);
                }
        ...
        if (bindingRevision > 0)
            variant.vertexArrayLookup = { revision: bindingRevision, vertexArray };
        return vertexArray;
    }
    /** 当前缓存了多少个 VAO（跨全部形态）。 */
    get vertexArrayCount() {
        ...
        for (const variant of this.variantCache.values())
            total += variant.vertexArrays.size;
        return total;
    }
    dispose() {
        if (this._disposed)
 ...
 */
function setAlphaToCoverage(gl, enabled) {
    if (enabled)
        gl.enable(GL_SAMPLE_ALPHA_TO_COVERAGE);
    else if (typeof gl.disable === 'function')
        gl.disable(GL_SAMPLE_ALPHA_TO_COVERAGE);
}
function buildVertexArrayKey(layouts, bindings, indexBuffer) {
    const parts = [];
    for (let slot = 0; slot < layouts.length; slot++) {
        const binding = bindings[slot];
        const layout = layouts[slot];
...
const indexBufferIds = new WeakMap();
let nextIndexBufferId = 1;
function indexBufferId(buffer) {
...
//# sourceMappingURL=WebGL2RenderPipeline.js.map