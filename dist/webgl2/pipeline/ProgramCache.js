...
import { DEFAULT_PREWARM_TIMEOUT_MS, createCompilationInfo, parseGlCompilationLog, nowMs, yieldToEventLoop, } from '../../core/pipeline/CompilationInfo.js';
import { glslTypeName, isSamplerType, reflectGlslProgram, } from '../../shaders/reflection/GLSLReflector.js';
/** 扩展缺失时的降级说明（`mode` 为 `'sync'` 时写进 `reason`）。 */
const PARALLEL_COMPILE_MISSING = 'KHR_parallel_shader_compile is not available on this WebGL2 context; gl.linkProgram() cannot be ' +
    'awaited asynchronously because querying LINK_STATUS blocks until linking finishes';
/** 同步 `acquire()` 链出来的 program 的说明。 */
const SYNC_ACQUIRE_REASON = 'the program was linked synchronously by ProgramCache.acquire() while the pipeline was created; ' +
...
export function programCacheKey(vertexSource, fragmentSource) {
...
export class ProgramCache {
     ...
     */
    acquire(label, vertexSource, fragmentSource) {
        ...
        const messages = [];
        const rawLogs = [];
        const vertexShader = this.compileShader(ShaderStage.Vertex, gl.VERTEX_SHADER, vertexSource, `${label} / vertex`, messages, rawLogs);
        const fragmentShader = this.compileShader(ShaderStage.Fragment, gl.FRAGMENT_SHADER, fragmentSource, `${label} / fragment`, messages, rawLogs);
        ...
        if (!linked) {
            ...
            throw new ValidationError(`[gpu-device-api] program「${label}」链接失败。vertex 与 fragment 的 varying（in/out）名字、\n` +
                '类型与数量必须完全对应。\n' +
                `GL 日志：${log}`);
        }
     ...
     */
    async compileAsync(label, vertexSource, fragmentSource, options = {}) {
        const key = programCacheKey(vertexSource, fragmentSource);
        const cached = this.programs.get(key);
        if (cached) {
            return {
    ...
    static toPrewarmResult(label, outcome) {
        return {
            label,
            backend: 'webgl2',
            ok: outcome.ok,
     ...
     * 计划要等链接完才能算出来。
     */
    bindPlan(compiled, plan) {
        if (compiled.boundPlanKey !== null)
            return;
        const { blockBindings, samplerLocations, optimizedOutBlocks } = this.bindResources(compiled.program, compiled.reflection, plan, compiled.label);
        compiled.blockBindings = blockBindings;
        compiled.samplerLocations = samplerLocations;
        compiled.optimizedOutBlocks = optimizedOutBlocks;
        compiled.boundPlanKey = plan ? plan.key : '';
    }
    /** 释放缓存里的全部 program。 */
    clear() {
    ...
    parallelCompile() {
        if (this.parallelExtension !== undefined)
            return this.parallelExtension;
        const extension = this.gl.getExtension('KHR_parallel_shader_compile');
    ...
    async linkAsync(label, vertexSource, fragmentSource, key, options) {
        const gl = this.gl;
        const started = nowMs();
        ...
        const mode = parallel ? 'async' : 'sync';
        const messages = [];
        const rawLogs = [];
        const fail = (reason) => ({
            compiled: null,
        ...
        try {
             ...
             */
            if (parallel) {
                const shadersReady = await this.waitFor(() => gl.getShaderParameter(vertexShader, parallel.COMPLETION_STATUS_KHR) &&
                    gl.getShaderParameter(fragmentShader, parallel.COMPLETION_STATUS_KHR), timeoutMs);
                if (!shadersReady) {
                    return fail(`shader compilation for "${label}" did not finish within ${String(timeoutMs)}ms ` +
                        '(polling COMPLETION_STATUS_KHR)');
                }
            }
            const vertexOk = gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS) === true;
            collectLog(gl.getShaderInfoLog(vertexShader), `${label} / vertex`, ShaderStage.Vertex, messages, rawLogs);
            const fragmentOk = gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS) === true;
            ...
            if (parallel) {
                const linkedReady = await this.waitFor(() => gl.getProgramParameter(program, parallel.COMPLETION_STATUS_KHR), timeoutMs);
                if (!linkedReady) {
                    return fail(`program linking for "${label}" did not finish within ${String(timeoutMs)}ms ` +
                        '(polling COMPLETION_STATUS_KHR)');
                }
            }
    ...
    createShaderObject(type, source, label) {
        const gl = this.gl;
    ...
    compileShader(stage, type, source, label, messages, rawLogs) {
        const gl = this.gl;
     ...
     */
    bindResources(program, reflection, plan, label) {
        const gl = this.gl;
        const blockBindings = new Map();
        const samplerLocations = new Map();
        const optimizedOutBlocks = [];
        // ---- uniform block -----------------------------------------------------------------------
        ...
        if (plan) {
            for (const [, slot] of plan.textures) {
                const location = samplerUniforms.find((uniform) => uniform.name === slot.name)?.location ??
        ...
        if (plan) {
            ...
            if (missingBlocks.length > 0) {
                throw new ValidationError(`[gpu-device-api] program「${label}」使用了未声明的 uniform block：${missingBlocks.join('、')}。\n` +
            ...
            if (missingSamplers.length > 0) {
                throw new ValidationError(`[gpu-device-api] program「${label}」使用了未声明的 sampler：${missingSamplers.join('、')}。\n` +
            ...
            if (reflection.uniformBlocks.length > 0) {
                throw new ValidationError(`[gpu-device-api] program「${label}」使用了 uniform block（${reflection.uniformBlocks
                    .map((block) => block.name)
                    .join('、')}），但管线没有声明任何 bind group layout。`);
            }
            if (samplerUniforms.length > 0) {
                throw new ValidationError(`[gpu-device-api] program「${label}」使用了 sampler（${samplerUniforms
...
function collectLog(log, label, stage, messages, rawLogs) {
    if (!log)
        return;
...
//# sourceMappingURL=ProgramCache.js.map