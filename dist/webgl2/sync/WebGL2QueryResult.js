...
export class WebGL2QueryResult {
    type;
    count;
    ...
    constructor(init) {
        ...
        this.yieldToEventLoop =
            init.yieldToEventLoop ?? (() => new Promise((resolve) => setTimeout(resolve, 0)));
    }
    async read() {
        if (this._read) {
            throw new ValidationError('[gpu-device-api] QueryResult.read(): this result has already been read. Call ' +
                'Device.readQuerySet() again for a fresh result.');
        }
        this._read = true;
        const gl = this.gl;
        if (this.querySet.disposed) {
            throw new ValidationError(`[gpu-device-api] QueryResult.read(): query set "${this.querySet.label}" has been destroyed.`);
        }
        ...
        for (let index = 0; index < this.count; index++) {
            ...
            const raw = gl.getQueryParameter(query, gl.QUERY_RESULT);
            values[index] = BigInt(Math.round(raw ?? 0));
        }
        return values;
    }
    /** 轮询 `QUERY_RESULT_AVAILABLE`，每次询问之间让出一拍。 */
    async waitUntilAvailable(query, index) {
        const gl = this.gl;
        ...
        for (;;) {
            ...
            if (this.querySet.disposed) {
                ...
                throw new GpuError(`[gpu-device-api] QueryResult.read(): query set "${this.querySet.label}" was destroyed while ` +
                    `waiting for query ${index}. The result of that frame is simply dropped.`, { code: 'QUERY_DISCARDED' });
            }
            if (Date.now() >= deadline) {
                throw new GpuError(`[gpu-device-api] QueryResult.read(): query ${index} of "${this.querySet.label}" was still not ` +
                    `available after ${this.timeoutMs} ms. The GL query is asynchronous; read it a few frames after ` +
                    'the pass that wrote it (gfx GPU timing waits `delay` frames for exactly this reason).', { code: 'QUERY_TIMEOUT' });
            }
            await this.yieldToEventLoop();
        }
    }
    assertNotDisjoint(index) {
        const extension = this.querySet.timerExtension;
        if (!extension)
            return;
        const disjoint = this.gl.getParameter(extension.GPU_DISJOINT_EXT);
        if (disjoint === true) {
            throw new GpuError(`[gpu-device-api] QueryResult.read(): GPU_DISJOINT_EXT is set, so the timer results of ` +
                `"${this.querySet.label}" (query ${index}) are undefined. A disjoint happens when the GPU is ` +
                'reset or preempted between the begin/end of the query; discard this sample instead of using it.', { code: 'QUERY_DISJOINT' });
        }
    }
}
/** 该结果是否属于 timestamp 查询（便于调用方判断单位）。 */
export function isTimestampQueryResult(result) {
    return result.type === QueryType.Timestamp;
}
//# sourceMappingURL=WebGL2QueryResult.js.map