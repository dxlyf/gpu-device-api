...
export const GL_PRIMITIVE_MODES = {
...
export const GL_BLEND_OPERATIONS = {
    add: 0x8006,
    subtract: 0x800a,
...
export const GL_CULL_FACES = {
    front: 0x0404,
    back: 0x0405,
};
export const GL_FRONT_FACES = {
    ccw: 0x0901,
    cw: 0x0900,
};
export const GL_ADDRESS_MODES = {
    'clamp-to-edge': 0x812f,
    repeat: 0x2901,
    'mirror-repeat': 0x8370,
};
export const GL_FILTERS = {
    nearest: 0x2600,
    linear: 0x2601,
};
 ...
 */
export function glVertexAttribute(format) {
...
export function glCompareFunc(compare) {
...
function parseCssColor(input) {
    ...
    throw new ValidationError(`[gpu-device-api] 无法解析颜色「${input}」。支持 CSS 十六进制、rgb()/rgba()、少量颜色名、0xRRGGBB、[r,g,b,a] 与 { r, g, b, a }。`);
}
/** WebGL2 不支持 storage buffer / storage texture，统一在这里给出解释。 */
export function assertNoStorageBinding(kind, binding, group) {
    throw new ValidationError(`[gpu-device-api] WebGL2 不支持 ${kind}（bind group ${group} 的 binding ${binding}）：` +
        'shader storage buffer / storage texture 需要 GLES 3.1 及以上，WebGL2 只有 GLES 3.0。' +
        '请把它移到 WebGPU 后端，或改用 uniform buffer + 纹理来传递数据。');
}
/** 检查颜色附件格式能否作为颜色目标。 */
export function assertColorAttachmentFormat(format) {
    // 这里只做「明显不可能」的拦截，真正的能力检测交给 glCapabilities 里的扩展查询。
    if (format.startsWith('depth') || format === 'stencil8') {
        throw new ValidationError(`[gpu-device-api] 纹理格式「${format}」是深度/模板格式，不能作为颜色附件。`);
    }
}
//# sourceMappingURL=glEnumMap.js.map