...
export const WEBGL2_SYNTHETIC_LIMITS = Object.freeze({
    maxBindGroups: 4,
...
function param(gl, name, fallback) {
...
export function queryGlLimits(gl) {
    return {
 ...
 */
export function buildDeviceLimits(gl) {
    ...
    return {
        ...
        maxDynamicUniformBuffersPerPipelineLayout: Math.min(WEBGL2_SYNTHETIC_LIMITS.maxDynamicUniformBuffersPerPipelineLayout, gl2.maxUniformBufferBindings),
        maxDynamicStorageBuffersPerPipelineLayout: 0,
...
export const WEBGL2_FEATURE_NAMES = [
    'texture-anisotropy',
...
/** 探测 WebGL2 实际可用的特性集合。 */
export function queryGlFeatures(gl) {
    const features = new Set();
    if (gl.getExtension('EXT_texture_filter_anisotropic'))
        features.add('texture-anisotropy');
    if (gl.getExtension('OES_texture_float_linear'))
        features.add('texture-float32-filterable');
    if (gl.getExtension('EXT_color_buffer_float'))
        features.add('color-buffer-float');
    if (gl.getExtension('WEBGL_debug_renderer_info'))
        features.add('debug-renderer-info');
...
export function queryGlRendererInfo(gl) {
    const debug = gl.getExtension('WEBGL_debug_renderer_info');
    if (!debug)
        return { vendor: '', device: '' };
    const vendor = gl.getParameter(debug.UNMASKED_VENDOR_WEBGL) ?? '';
    const device = gl.getParameter(debug.UNMASKED_RENDERER_WEBGL) ?? '';
    return { vendor, device };
}
/** 查询纹理各向异性上限；扩展不可用时返回 1。 */
export function queryMaxAnisotropy(gl) {
    const extension = gl.getExtension('EXT_texture_filter_anisotropic');
    if (!extension)
        return 1;
    return gl.getParameter(extension.MAX_TEXTURE_MAX_ANISOTROPY_EXT) ?? 1;
}
/** 取 WebGL2 context；拿不到时给出人类可读的原因。 */
export function requireWebGL2Context(canvas, attributes) {
    const context = canvas.getContext('webgl2', attributes);
    if (!context) {
        throw new ValidationError('[gpu-device-api] 无法创建 WebGL2 context。常见原因：浏览器不支持 WebGL2、' +
...
export function multisampleApi(gl) {
    return gl;
}
//# sourceMappingURL=glCapabilities.js.map