...
const GL_TYPE_ELEMENT_BYTES = new Map([
 ...
 */
export function glTypeElementBytes(type) {
    const bytes = GL_TYPE_ELEMENT_BYTES.get(type);
    if (bytes === undefined) {
        throw new ValidationError(`[gpu-device-api] 未知的 GL 像素类型 0x${type.toString(16)}，无法计算上传布局。`);
    }
    return bytes;
}
/** GL 像素类型对应的主机 TypedArray 名字（错误消息用）。 */
export function glTypeArrayName(type) {
    return GL_TYPE_NAMES.get(type) ?? 'TypedArray';
}
/** 两个方向共用的校验部分（`bytesPerRow` 的整除性只对上传方向有约束）。 */
function resolveCommon(layout, size, info, operation, format, label) {
    ...
    if (!Number.isInteger(bytesPerRow) || bytesPerRow < tightRowBytes) {
        throw new ValidationError(`[gpu-device-api] ${operation}: bytesPerRow must be an integer >= ${tightRowBytes} ` +
    ...
    if (!Number.isInteger(rowsPerImage) || rowsPerImage < size.height) {
        throw new ValidationError(`[gpu-device-api] ${operation}: rowsPerImage (${String(rowsPerImage)}) must be >= the copy height ` +
            `(${size.height}).${where}`);
    }
    if (!Number.isInteger(size.depthOrArrayLayers) || size.depthOrArrayLayers < 1) {
        throw new ValidationError(`[gpu-device-api] ${operation}: depthOrArrayLayers must be a positive integer, ` +
 ...
 */
export function resolveUploadLayout(layout, size, info, operation, format, label, availableBytes) {
    const { bytesPerRow, rowsPerImage, bytesPerPixel, tightRowBytes, where } = resolveCommon(layout, size, info, operation, format, label);
    /*
    ...
    if (availableBytes !== undefined && availableBytes < requiredBytes) {
        throw new ValidationError(`[gpu-device-api] ${operation}: the source data is too small for ${size.width}x${size.height}` +
 ...
 * 哪个偏移写」的问题。
 */
export function resolveReadbackLayout(layout, size, info, operation, format, label) {
    const { bytesPerRow, rowsPerImage, bytesPerPixel } = resolveCommon(layout, size, info, operation, format, label);
    /*
 ...
 */
export function toUploadElementView(format, info, data, operation, scratch) {
    ...
    if (view.byteOffset % elementBytes === 0) {
        ...
        return createTypedArray(info.type, view.buffer, view.byteOffset, usable);
    }
    /*
    ...
    if (!scratch || scratch.byteLength < view.byteLength) {
        throw new ValidationError(`[gpu-device-api] ${operation}: the pixel data starts at byte offset ${view.byteOffset} of its ` +
    ...
    raw.set(new Uint8Array(view.buffer, view.byteOffset, view.byteLength));
    const usable = view.byteLength - (view.byteLength % elementBytes);
    return createTypedArray(info.type, scratch, 0, usable);
}
/** 按 GL 像素类型造一个对应类型的视图（长度按**元素个数**算）。 */
export function createTypedArray(type, buffer, byteOffset, byteLength) {
    switch (type) {
        ...
        default:
            throw new ValidationError(`[gpu-device-api] 未知的 GL 像素类型 0x${type.toString(16)}，无法构造上传视图。`);
    }
}
/**
 ...
 */
export function uploadTextureData(gl, target, native, mipLevel, origin, size, info, format, data, layout, scratchProvider, 
...
operation = 'writeTexture') {
    ...
    const view = (byteOffset, byteLength) => toUploadElementView(format, info, new Uint8Array(data.buffer, data.byteOffset + byteOffset, byteLength), operation, scratchProvider());
    gl.bindTexture(target, native);
    gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1);
    if (layout.unpackRowLength !== 0)
        gl.pixelStorei(GL_UNPACK_ROW_LENGTH, layout.unpackRowLength);
    if (layout.unpackImageHeight !== 0)
        gl.pixelStorei(GL_UNPACK_IMAGE_HEIGHT, layout.unpackImageHeight);
    try {
        if (layout.perRowUpload) {
            ...
            for (let layer = 0; layer < size.depthOrArrayLayers; layer += 1) {
                for (let row = 0; row < size.height; row += 1) {
                    ...
                    if (layered) {
                        gl.texSubImage3D(target, mipLevel, origin.x, origin.y, origin.z + layer, size.width, 1, 1, info.format, info.type, source);
                    }
                    else {
                        gl.texSubImage2D(target, mipLevel, origin.x, origin.y + row, size.width, 1, info.format, info.type, source);
                    }
                }
            }
        }
        else if (layered) {
            gl.texSubImage3D(target, mipLevel, origin.x, origin.y, origin.z, size.width, size.height, size.depthOrArrayLayers, info.format, info.type, view(0, data.byteLength));
        }
        else {
            gl.texSubImage2D(target, mipLevel, origin.x, origin.y, size.width, size.height, info.format, info.type, view(0, data.byteLength));
        }
    }
    finally {
 ...
 */
export function repackRows(tight, out, offset, tightRowBytes, bytesPerRow, height) {
...
//# sourceMappingURL=copyLayout.js.map