 ...
 */
function assertAllBlendFieldsCompared() {
    return null;
}
assertAllBlendFieldsCompared();
export class GlStateCache {
    ...
    uniformBufferOffsetAlignment() {
        if (this.uniformAlignment === null) {
            const value = this.gl.getParameter(this.gl.UNIFORM_BUFFER_OFFSET_ALIGNMENT);
     ...
     * 免得之后被迫整体失效。
     */
    noteFramebufferBinding(framebuffer) {
    ...
    bindFramebuffer(framebuffer) {
        if (this.framebufferBinding !== undefined && this.framebufferBinding === framebuffer)
            return;
    ...
    currentFramebuffer() {
        if (this.framebufferBinding === undefined) {
            this.framebufferBinding = this.gl.getParameter(this.gl.FRAMEBUFFER_BINDING);
        }
    ...
    readbackFramebuffer() {
        ...
        if (!framebuffer) {
            throw new ValidationError('[gpu-device-api] gl.createFramebuffer() 返回 null，无法创建读回用的 framebuffer。');
        }
     ...
     */
    attachReadbackTexture(texture, attachment, mipLevel) {
        ...
        if (previous && previous.texture === texture && previous.attachment === attachment)
            return;
    ...
    invalidateTextureUnits() {
        ...
        if (this.activeUnit >= 0)
            this.textures.delete(this.activeUnit);
    }
    useProgram(program) {
    ...
    bindTexture(unit, target, texture) {
        ...
        if (current && current.target === target && current.texture === texture)
            return;
    ...
    bindUniformBuffer(index, buffer, offset = 0, size = -1) {
        const current = this.uniformBuffers.get(index);
        if (current && current.buffer === buffer && current.offset === offset && current.size === size)
            return;
        if (size < 0) {
     ...
     * 代码直接编译不过。
     */
    setBlend(enabled, colorSrc, colorDst, colorOp, alphaSrc, alphaDst, alphaOp) {
    ...
    setBlendConstant(color) {
        if (same4(this.blendConstant, color))
            return;
     ...
     */
    setDepthTest(enabled, write, func, bias) {
        const gl = this.gl;
     ...
     */
    setStencilTest(state) {
        const gl = this.gl;
        const previous = this.stencil;
        if (previous !== null && sameStencilState(previous, state))
            return;
    ...
    setCull(enabled, face, frontFace) {
        const gl = this.gl;
    ...
    setViewport(x, y, width, height) {
        if (same4(this.viewport, [x, y, width, height]))
            return;
     ...
     */
    setScissor(enabled, x, y, width, height) {
        const gl = this.gl;
        // 记住「这个尺寸的整个附件」是什么，供 {@link resetScissor} 免掉多余的 GL 调用。
        if (enabled)
            this.scissorWholeBox = [0, 0, width, height];
     ...
     */
    resetScissor(width, height) {
        const gl = this.gl;
        const wholeBox = [0, 0, width, height];
        /*
         * 三个条件都要满足才算「已经在复位状态上」：
         * - 开关是关的；
         * - 我们记下的矩形就是「整个附件」；
...
//# sourceMappingURL=glStateCache.js.map