 ...
 */
export function formatGlErrorCode(code) {
 ...
 * 其余一律当 `'validation'`。
 */
export function classifyGlError(error) {
 ...
 */
export function createGlError(code, context) {
    const hex = formatGlErrorCode(code);
    const details = { glError: code, context };
    if (code === GL_ERROR_CODES.OUT_OF_MEMORY) {
        return new OutOfMemoryError(`[gpu-device-api] GL out of memory (${hex}) observed during ${context}.`, { details });
    }
    if (code === GL_ERROR_CODES.INVALID_ENUM ||
        code === GL_ERROR_CODES.INVALID_VALUE ||
        code === GL_ERROR_CODES.INVALID_OPERATION ||
        code === GL_ERROR_CODES.INVALID_FRAMEBUFFER_OPERATION) {
        return new ValidationError(`[gpu-device-api] GL error ${hex} observed during ${context}. This code means "the API was used ` +
            'incorrectly", but GL does not tell which call produced it, nor whether the real cause was ' +
            'validation, memory or the implementation itself.', { details });
    }
    if (code === GL_ERROR_CODES.CONTEXT_LOST_WEBGL) {
        return new GPUInternalError(`[gpu-device-api] GL context lost (${hex}) observed during ${context}. Every GL object created by ` +
            'this device is now invalid; recreate the device and its resources.', { details });
    }
    return new GPUInternalError(`[gpu-device-api] unknown GL error ${hex} observed during ${context}. GL does not define this code, ` +
        'so it comes from the driver or a vendor extension and cannot be classified.', { details });
}
/**
 * WebGL2 的错误作用域句柄。
 *
...
//# sourceMappingURL=glErrorScope.js.map