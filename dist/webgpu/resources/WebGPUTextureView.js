...
import { DEFAULT_TEXTURE_SWIZZLE, resolveTextureViewDescriptor, } from '../../core/resources/TextureView.js';
...
export class WebGPUTextureView {
    label;
    texture;
    descriptor;
    native;
     ...
     */
    constructor(texture, descriptor = {}, preResolved) {
        ...
        if (resolved.baseMipLevel + resolved.mipLevelCount > texture.mipLevelCount) {
            throw new ValidationError(`[gpu-device-api] Texture "${texture.label}".createView: mip range ` +
        ...
        if (resolved.baseArrayLayer + resolved.arrayLayerCount > texture.depthOrArrayLayers) {
            throw new ValidationError(`[gpu-device-api] Texture "${texture.label}".createView: array layer range ` +
        ...
        if (resolved.dimension === 'cube' || resolved.dimension === 'cube-array') {
            if (resolved.arrayLayerCount % 6 !== 0) {
                throw new ValidationError(`[gpu-device-api] Texture "${texture.label}".createView: a "${resolved.dimension}" view needs a ` +
        ...
        if (resolved.format !== undefined) {
            ...
            if (resolved.format !== texture.format && !texture.viewFormats.includes(resolved.format)) {
                throw new ValidationError(`[gpu-device-api] Texture "${texture.label}".createView: view format "${resolved.format}" was not ` +
...
export function isWebGPUTextureView(value) {
    return value instanceof WebGPUTextureView;
}
/** 原生 `GPUTextureView` 的形状识别：它没有任何自有方法，只能靠 `@@toStringTag` 判断。 */
export function isNativeGPUTextureView(value) {
    if (!value || typeof value !== 'object')
        return false;
    const candidate = value;
    if ('native' in candidate || 'createView' in candidate)
        return false;
    if (Object.prototype.toString.call(value) === '[object GPUTextureView]')
        return true;
 ...
 */
export function asGPUTextureView(value, context) {
    if (value instanceof WebGPUTextureView)
        return value.native;
    if (isNativeGPUTextureView(value))
        return value;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU texture view (WebGPUTextureView or a native ` +
        `GPUTextureView), got ${describeUnknown(value)}.`);
}
//# sourceMappingURL=WebGPUTextureView.js.map