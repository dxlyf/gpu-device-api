...
export class WebGPUShaderModule {
    ...
    constructor(device, descriptor) {
        ...
        if (this.source.wgsl === undefined && this.source.vs === undefined && this.source.fs === undefined && this.source.cs === undefined) {
            throw new ValidationError(`[gpu-device-api] ShaderModule "${this.label}" has no source code (expected \`wgsl\`, or \`vs\`/\`fs\`/\`cs\`).`);
        }
    }
    get disposed() {
    ...
    compile(stage) {
        if (this._disposed) {
            throw new ValidationError(`[gpu-device-api] ShaderModule "${this.label}" has been disposed; it can no longer be compiled.`);
        }
        if (this.device.disposed) {
            throw new ValidationError(`[gpu-device-api] ShaderModule "${this.label}" belongs to a disposed device.`);
        }
     ...
     */
    async getCompilationInfo(stage = ShaderStageEnum.Vertex) {
            ...
            const info = createCompilationInfo({
                ...
                messages: [
                    createCompilationMessage({
                        type: 'info',
                        message: 'this WebGPU implementation does not expose GPUShaderModule.getCompilationInfo(); ' +
        ...
        const info = createCompilationInfo({
            label: this.label,
            backend: 'webgpu',
            messages: compiled.messages.map((message) => createCompilationMessage({
...
export function isWebGPUShaderModule(value) {
    return value instanceof WebGPUShaderModule;
}
/** 把 core 的 `ShaderModule` 收窄为 WebGPU 实现。 */
export function asWebGPUShaderModule(value, context) {
    if (value instanceof WebGPUShaderModule)
        return value;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPUShaderModule created by this device, got ` +
        `${describeUnknown(value)}.`);
}
//# sourceMappingURL=WebGPUShaderModule.js.map