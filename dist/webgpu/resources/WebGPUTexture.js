...
import { assertRenderableFormat, assertTextureUsageSupported, fromGPUTextureFormat, isFilterableFormat, toGPUTextureFormat, } from '../utils/wgpuFormatMap.js';
...
export class WebGPUTexture {
     ...
     */
    mipPassCache = new Map();
    _disposed = false;
    constructor(device, native, descriptor, owned) {
        this.device = device;
        this.owned = owned;
        this.native = native;
     ...
     */
    static adopt(device, native, descriptor = {}, options = {}) {
    ...
    createView(descriptor) {
        if (this._disposed) {
            throw new ValidationError(`[gpu-device-api] Texture.createView: texture "${this.label}" has been destroyed.`);
        }
    ...
    generateMipmaps() {
        ...
        if (!this.usable) {
            throw new ValidationError(`[gpu-device-api] Texture "${this.label}".generateMipmaps: the owning device is not usable.`);
        }
        ...
        if ((this.usage & TextureUsage.RenderAttachment) === 0) {
            throw new ValidationError(`[gpu-device-api] Texture "${this.label}".generateMipmaps: WebGPU has no generateMipmap, so the ` +
        ...
        if (!isFilterableFormat(this.format, this.device.features)) {
            throw new ValidationError(`[gpu-device-api] Texture "${this.label}".generateMipmaps: format "${this.format}" is not ` +
     ...
     * 永远与当前的管线布局匹配。
     */
    acquireMipPass(device, generator, level, layer, layered) {
        ...
        if (!layered) {
            ...
            const pass = { source, target, bindGroup, pipeline: generator.pipeline };
...
function viewCacheKey(d) {
    return cacheKey(d.format ?? '', d.dimension, d.baseMipLevel, d.mipLevelCount, d.baseArrayLayer, d.arrayLayerCount, d.aspect, 
...
const mipmapGenerators = new WeakMap();
/** 着色器模块与格式无关，每个 device 每种变体只建一次。 */
const mipmapShaderModules = new WeakMap();
/** 缓存键：**级与层都要在键里**，少一个字段就会跨层串味（见 `WebGPUTexture.mipPassCache`）。 */
function mipPassCacheKey(level, layer) {
    return `${level}:${layer}`;
}
/**
 ...
 */
function layerCountAtLevel(dimension, depthOrArrayLayers, level) {
    if (dimension === '3d')
        return Math.max(1, depthOrArrayLayers >> level);
 ...
 */
function acquireMipmapGenerator(device, format) {
...
function assertTextureDescriptor(descriptor, device) {
    const { width, height, depthOrArrayLayers } = descriptor.size;
    if (!Number.isInteger(width) || width <= 0 || !Number.isInteger(height) || height <= 0) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": width and height must be positive integers, ` +
            `got ${width}x${height}.`);
    }
    if (!Number.isInteger(depthOrArrayLayers) || depthOrArrayLayers <= 0) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": depthOrArrayLayers must be a positive integer, ` +
            `got ${String(depthOrArrayLayers)}.`);
    }
    if (!Number.isInteger(descriptor.mipLevelCount) || descriptor.mipLevelCount <= 0) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": mipLevelCount must be a positive integer.`);
    }
    const maxMipLevels = fullMipLevelCount(descriptor.size);
    if (descriptor.mipLevelCount > maxMipLevels) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": mipLevelCount ${descriptor.mipLevelCount} is more than ` +
    ...
    if (descriptor.dimension === '1d' && height !== 1) {
        ...
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": a "1d" texture must have height 1, got ${height}.`);
    }
    if (descriptor.dimension === '1d' && descriptor.sampleCount > 1) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": a "1d" texture cannot be multisampled.`);
    }
    const maxDimension = descriptor.dimension === '1d'
    ...
    if (width > maxDimension || height > maxDimension || depthOrArrayLayers > maxDimension) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": ${width}x${height}x${depthOrArrayLayers} exceeds the ` +
            `${descriptor.dimension} limit ${maxDimension}.`);
    }
    if (descriptor.dimension === '2d' && depthOrArrayLayers > device.limits.maxTextureArrayLayers) {
        throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": depthOrArrayLayers ${depthOrArrayLayers} exceeds ` +
            `maxTextureArrayLayers ${device.limits.maxTextureArrayLayers}.`);
    }
    assertTextureUsageSupported(descriptor.format, descriptor.usage, {
    ...
    for (const viewFormat of descriptor.viewFormats) {
        ...
        const strip = (format) => format.replace('-srgb', '');
        if (strip(viewFormat) !== strip(descriptor.format)) {
            throw new ValidationError(`[gpu-device-api] Texture "${descriptor.label}": viewFormat "${viewFormat}" is not compatible with ` +
                `format "${descriptor.format}" (WebGPU only allows srgb <-> non-srgb reinterpretation).`);
        }
    }
}
/** 该对象是否为 WebGPU 后端的 texture。 */
export function isWebGPUTexture(value) {
    return value instanceof WebGPUTexture;
}
/** 原生 `GPUTexture` 的形状识别。 */
export function isNativeGPUTexture(value) {
 ...
 */
export function asGPUTexture(value, context) {
    if (value instanceof WebGPUTexture)
        return value.native;
    if (isNativeGPUTexture(value))
        return value;
    const nested = value?.native;
    if (nested !== undefined && isNativeGPUTexture(nested))
        return nested;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU texture (WebGPUTexture or a native GPUTexture), ` +
        `got ${describeUnknown(value)}.`);
}
//# sourceMappingURL=WebGPUTexture.js.map