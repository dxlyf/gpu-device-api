 ...
 * 两个都认，避免「实现明明支持却报不支持」。
 */
export const TIMESTAMP_INSIDE_PASSES_FEATURES = [
...
export class WebGPUQuerySet {
    ...
    constructor(device, descriptor) {
        ...
        if (descriptor.type === QueryTypeValues.Timestamp && !device.hasEnabledFeature(TIMESTAMP_QUERY_FEATURE)) {
            throw new ValidationError(`[gpu-device-api] QuerySet "${this.label}": timestamp queries need the "${TIMESTAMP_QUERY_FEATURE}" ` +
...
export function isWebGPUQuerySet(value) {
    return value instanceof WebGPUQuerySet;
}
/** 把任意 query set 表示收窄为原生 `GPUQuerySet`。 */
export function asGPUQuerySet(value, context) {
    if (value instanceof WebGPUQuerySet)
        return value.native;
    ...
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU query set (WebGPUQuerySet or a native GPUQuerySet).`);
}
/**
 ...
 */
export function toGPUTimestampWrites(writes, device, context) {
    assertPassTimestampWrites(writes, context);
    if (!device.hasEnabledFeature(TIMESTAMP_QUERY_FEATURE)) {
        throw new ValidationError(`[gpu-device-api] ${context}: writing timestamps inside a pass needs the "${TIMESTAMP_QUERY_FEATURE}" ` +
    ...
    if (insidePasses === undefined) {
        throw new ValidationError(`[gpu-device-api] ${context}: this WebGPU implementation does not enable ` +
