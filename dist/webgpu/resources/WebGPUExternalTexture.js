...
export class WebGPUExternalTexture {
    label;
    native;
    device;
    _disposed = false;
    constructor(device, native, label) {
        this.device = device;
        this.native = native;
        this.label = label;
    }
     ...
     */
    get expired() {
        return this.native.expired === true;
    }
    get disposed() {
 ...
 */
export function isWebGPUExternalTexture(value) {
    return value instanceof WebGPUExternalTexture;
}
/** 原生 `GPUExternalTexture` 的形状识别（它没有方法可鸭子类型判断，只能靠 `@@toStringTag`）。 */
export function isNativeGPUExternalTexture(value) {
    if (!value || typeof value !== 'object')
        return false;
    const candidate = value;
    if ('native' in candidate)
        return false;
 ...
 */
export function asGPUExternalTexture(value, context) {
    if (value instanceof WebGPUExternalTexture)
        return value.native;
    if (isNativeGPUExternalTexture(value))
        return value;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU external texture (WebGPUExternalTexture or a native ` +
