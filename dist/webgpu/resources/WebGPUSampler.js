...
export class WebGPUSampler {
    ...
    constructor(device, descriptor = {}) {
        ...
        if (!Number.isFinite(resolved.lodMinClamp) || !Number.isFinite(resolved.lodMaxClamp)) {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": lodMinClamp/lodMaxClamp must be finite numbers.`);
        }
        if (resolved.lodMinClamp < 0) {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": lodMinClamp must not be negative, got ${resolved.lodMinClamp}.`);
        }
        if (resolved.lodMaxClamp < resolved.lodMinClamp) {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": lodMaxClamp (${resolved.lodMaxClamp}) must be >= ` +
                `lodMinClamp (${resolved.lodMinClamp}).`);
        }
        if (!Number.isFinite(resolved.maxAnisotropy) || resolved.maxAnisotropy < 1) {
            throw new ValidationError(`[gpu-device-api] Sampler "${this.label}": maxAnisotropy must be >= 1, got ${String(resolved.maxAnisotropy)}.`);
        }
...
export function isWebGPUSampler(value) {
    return value instanceof WebGPUSampler;
}
/** 原生 `GPUSampler` 的形状识别。 */
export function isNativeGPUSampler(value) {
...
export function asGPUSampler(value, context) {
    if (value instanceof WebGPUSampler)
        return value.native;
    if (isNativeGPUSampler(value))
        return value;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU sampler (WebGPUSampler or a native GPUSampler).`);
}
