...
const KNOWN_SHADER_STAGE = GPU_SHADER_STAGE.VERTEX | GPU_SHADER_STAGE.FRAGMENT | GPU_SHADER_STAGE.COMPUTE;
const KNOWN_BUFFER_USAGE = GPU_BUFFER_USAGE.MAP_READ |
...
const KNOWN_TEXTURE_USAGE = GPU_TEXTURE_USAGE.COPY_SRC |
 ...
 */
export function toGPUShaderStage(visibility) {
    if (!Number.isInteger(visibility)) {
        throw new ValidationError(`[gpu-device-api] ShaderStage visibility must be an integer bit mask, got ${String(visibility)}.`);
    }
    if ((visibility & ~KNOWN_SHADER_STAGE) !== 0) {
        throw new ValidationError(`[gpu-device-api] ShaderStage visibility 0x${(visibility >>> 0).toString(16)} contains unknown bits; ` +
            'expected a combination of Vertex (0x1), Fragment (0x2) and Compute (0x4).');
    }
    let flags = 0;
    if ((visibility & ShaderStage.Vertex) !== 0)
        flags |= GPU_SHADER_STAGE.VERTEX;
    if ((visibility & ShaderStage.Fragment) !== 0)
        flags |= GPU_SHADER_STAGE.FRAGMENT;
    if ((visibility & ShaderStage.Compute) !== 0)
        flags |= GPU_SHADER_STAGE.COMPUTE;
    if (flags === 0) {
        throw new ValidationError('[gpu-device-api] A BindGroupLayout entry must be visible from at least one shader stage.');
    }
    return flags;
}
/** `ColorWriteMask` → `GPUColorWriteFlags`。位定义与 WebGPU 完全一致。 */
export function toGPUColorWriteMask(mask) {
    if (!Number.isInteger(mask) || (mask & ~GPU_COLOR_WRITE.ALL) !== 0) {
        throw new ValidationError(`[gpu-device-api] ColorWriteMask must be a combination of Red (0x1), Green (0x2), Blue (0x4) ` +
            `and Alpha (0x8); got ${String(mask)}.`);
    }
    // 值一一对应，但仍逐位翻译，避免以后任一侧改动时静默错位。
    let flags = 0;
    if ((mask & ColorWriteMask.Red) !== 0)
        flags |= GPU_COLOR_WRITE.RED;
    if ((mask & ColorWriteMask.Green) !== 0)
        flags |= GPU_COLOR_WRITE.GREEN;
    if ((mask & ColorWriteMask.Blue) !== 0)
        flags |= GPU_COLOR_WRITE.BLUE;
    if ((mask & ColorWriteMask.Alpha) !== 0)
        flags |= GPU_COLOR_WRITE.ALPHA;
    return flags;
}
/** `BufferUsage` → `GPUBufferUsageFlags`。 */
export function toGPUBufferUsage(usage) {
    if (!Number.isInteger(usage) || usage <= 0) {
        throw new ValidationError(`[gpu-device-api] BufferUsage must be a non-empty combination of usage flags, got ${String(usage)}.`);
    }
    if ((usage & ~KNOWN_BUFFER_USAGE) !== 0) {
        throw new ValidationError(`[gpu-device-api] BufferUsage 0x${(usage >>> 0).toString(16)} contains unknown bits.`);
    }
    if ((usage & BufferUsage.MapRead) !== 0 && (usage & BufferUsage.MapWrite) !== 0) {
        throw new ValidationError('[gpu-device-api] BufferUsage cannot combine MapRead and MapWrite: WebGPU rejects a buffer that is ' +
            'both mappable for reading and for writing. Use two buffers (or CopyDst plus writeBuffer).');
    }
    let flags = 0;
    if ((usage & BufferUsage.MapRead) !== 0)
        flags |= GPU_BUFFER_USAGE.MAP_READ;
    if ((usage & BufferUsage.MapWrite) !== 0)
        flags |= GPU_BUFFER_USAGE.MAP_WRITE;
    if ((usage & BufferUsage.CopySrc) !== 0)
        flags |= GPU_BUFFER_USAGE.COPY_SRC;
    if ((usage & BufferUsage.CopyDst) !== 0)
        flags |= GPU_BUFFER_USAGE.COPY_DST;
    if ((usage & BufferUsage.Index) !== 0)
        flags |= GPU_BUFFER_USAGE.INDEX;
    if ((usage & BufferUsage.Vertex) !== 0)
        flags |= GPU_BUFFER_USAGE.VERTEX;
    if ((usage & BufferUsage.Uniform) !== 0)
        flags |= GPU_BUFFER_USAGE.UNIFORM;
    if ((usage & BufferUsage.Storage) !== 0)
        flags |= GPU_BUFFER_USAGE.STORAGE;
    if ((usage & BufferUsage.Indirect) !== 0)
        flags |= GPU_BUFFER_USAGE.INDIRECT;
    if ((usage & BufferUsage.QueryResolve) !== 0)
        flags |= GPU_BUFFER_USAGE.QUERY_RESOLVE;
    return flags;
}
/** `TextureUsage` → `GPUTextureUsageFlags`。格式相关的限制由 `wgpuFormatMap` 负责。 */
export function toGPUTextureUsage(usage) {
    if (!Number.isInteger(usage) || usage <= 0) {
        throw new ValidationError(`[gpu-device-api] TextureUsage must be a non-empty combination of usage flags, got ${String(usage)}.`);
    }
    if ((usage & ~KNOWN_TEXTURE_USAGE) !== 0) {
        throw new ValidationError(`[gpu-device-api] TextureUsage 0x${(usage >>> 0).toString(16)} contains unknown bits.`);
    }
    let flags = 0;
    if ((usage & TextureUsage.CopySrc) !== 0)
        flags |= GPU_TEXTURE_USAGE.COPY_SRC;
    if ((usage & TextureUsage.CopyDst) !== 0)
        flags |= GPU_TEXTURE_USAGE.COPY_DST;
    if ((usage & TextureUsage.TextureBinding) !== 0)
        flags |= GPU_TEXTURE_USAGE.TEXTURE_BINDING;
    if ((usage & TextureUsage.StorageBinding) !== 0)
        flags |= GPU_TEXTURE_USAGE.STORAGE_BINDING;
    if ((usage & TextureUsage.RenderAttachment) !== 0)
        flags |= GPU_TEXTURE_USAGE.RENDER_ATTACHMENT;
    return flags;
}
/** `MapMode` → `GPUMapModeFlags`。 */
export function toGPUMapMode(mode) {
    switch (mode) {
        case 'read':
...
export function toGPUPrimitiveTopology(topology) {
    switch (topology) {
        case 'point-list':
            return 'point-list';
        case 'line-list':
...
export function toGPUIndexFormat(format) {
    switch (format) {
        case 'uint16':
            return 'uint16';
        case 'uint32':
...
export function toGPUCompareFunction(compare) {
    switch (compare) {
        case 'never':
            return 'never';
        case 'less':
...
export function toGPUStencilOperation(operation) {
    switch (operation) {
        case 'keep':
            return 'keep';
        case 'zero':
...
export function toGPUBlendFactor(factor) {
    switch (factor) {
        case 'zero':
            return 'zero';
        case 'one':
...
export function toGPUBlendOperation(operation) {
    switch (operation) {
        case 'add':
            return 'add';
        case 'subtract':
...
export function toGPUAddressMode(mode) {
    switch (mode) {
        case 'clamp-to-edge':
...
export function toGPUFilterMode(mode) {
    switch (mode) {
        case 'nearest':
            return 'nearest';
        case 'linear':
...
export function toGPUMipmapFilterMode(mode) {
    switch (mode) {
        case 'nearest':
            return 'nearest';
        case 'linear':
...
export function toGPUCullMode(mode) {
    switch (mode) {
        case 'none':
            return 'none';
        case 'front':
...
export function toGPUFrontFace(face) {
    switch (face) {
        case 'ccw':
            return 'ccw';
        case 'cw':
...
/** `LoadOp` → `GPULoadOp`。 */
export function toGPULoadOp(loadOp) {
    switch (loadOp) {
        case 'load':
            return 'load';
        case 'clear':
...
/** `StoreOp` → `GPUStoreOp`。 */
export function toGPUStoreOp(storeOp) {
    switch (storeOp) {
        case 'store':
            return 'store';
        case 'discard':
...
export function toGPUTextureViewDimension(dimension) {
    switch (dimension) {
        case '1d':
            return '1d';
        case '2d':
...
export function toGPUTextureAspect(aspect) {
    switch (aspect) {
        case 'all':
            return 'all';
        case 'depth-only':
...
export function toGPUVertexStepMode(mode) {
    switch (mode) {
        case 'vertex':
            return 'vertex';
        case 'instance':
 ...
 */
export function toGPUVertexFormat(format) {
    switch (format) {
        // 8 位
        case 'uint8x2':
            return 'uint8x2';
...
export function toGPUQueryType(type) {
    switch (type) {
...
export function toGPUBufferBindingType(type) {
    switch (type) {
        case 'uniform':
            return 'uniform';
        case 'storage':
        ...
        default:
            throw new ValidationError(`[gpu-device-api] BindingType "${String(type)}" is not a buffer binding; ` +
                'expected "uniform", "storage" or "read-only-storage".');
    }
}
/** `BindingType` → `GPUSamplerBindingType`；非 sampler 绑定会抛错。 */
export function toGPUSamplerBindingType(type) {
    switch (type) {
        case 'sampler':
            return 'filtering';
        case 'comparison-sampler':
            return 'comparison';
        default:
            throw new ValidationError(`[gpu-device-api] BindingType "${String(type)}" is not a sampler binding; ` +
                'expected "sampler" or "comparison-sampler".');
    }
}
/** `TextureSampleType` → `GPUTextureSampleType`。 */
export function toGPUTextureSampleType(sampleType) {
    switch (sampleType) {
        case 'float':
            return 'float';
...
export function toGPUStorageTextureAccess(access) {
    switch (access) {
        case 'write-only':
            return 'write-only';
        case 'read-only':
...
export function assertSamplerBindingType(type) {
    switch (type) {
        case 'filtering':
        case 'non-filtering':
        case 'comparison':
 ...
 */
export function resolveClearColor(color) {
    if (color === undefined)
    ...
    if (Array.isArray(color)) {
        ...
        if (values.length !== 3 && values.length !== 4) {
            throw new ValidationError(`[gpu-device-api] A clear color array needs 3 or 4 components, got ${values.length}.`);
        }
    ...
    if (typeof dict.r !== 'number' || typeof dict.g !== 'number' || typeof dict.b !== 'number') {
        throw new ValidationError('[gpu-device-api] A clear color object needs numeric "r", "g" and "b" members.');
    }
    return makeColor(dict.r, dict.g, dict.b, typeof dict.a === 'number' ? dict.a : 1, color);
}
function makeColor(r, g, b, a, source) {
    for (const [name, value] of [
        ...
        if (!Number.isFinite(value)) {
            throw new ValidationError(`[gpu-device-api] Clear color component "${name}" must be a finite number, got ${String(value)} ` +
...
function parseColorNumber(value) {
    if (!Number.isFinite(value) || !Number.isInteger(value) || value < 0 || value > 0xffffff) {
        throw new ValidationError(`[gpu-device-api] A numeric clear color must be an integer in 0x000000..0xFFFFFF (0xRRGGBB), got ${String(value)}.`);
    }
    return {
        r: ((value >> 16) & 0xff) / 255,
        g: ((value >> 8) & 0xff) / 255,
...
function parseColorString(input) {
    ...
    if (named)
        return { r: named[0], g: named[1], b: named[2], a: named[3] };
    const functional = FUNCTIONAL_COLOR.exec(text);
    if (functional) {
        const parts = functional[1].replace(/\//g, ' ').split(/[\s,]+/).filter((part) => part.length > 0);
        if (parts.length !== 3 && parts.length !== 4) {
            throw new ValidationError(`[gpu-device-api] Clear color "${input}" needs 3 or 4 components inside rgb()/rgba().`);
        }
    ...
    throw new ValidationError(`[gpu-device-api] Unsupported clear color string "${input}". Expected "#rgb", "#rgba", "#rrggbb", ` +
        '"#rrggbbaa", "rgb()/rgba()", a known color name, a 0xRRGGBB number, an array of 0..1 components, ' +
        'or an { r, g, b, a } object.');
}
function parseRgbComponent(part, source) {
    if (part.endsWith('%')) {
...
function parseAlphaComponent(part, source) {
    if (part.endsWith('%')) {
        const percent = Number(part.slice(0, -1));
        if (!Number.isFinite(percent)) {
...
export function toGPUExtent3D(extent) {
    return {
        width: extent.width,
...
export function toGPUOrigin3D(origin) {
    return {
        x: origin?.x ?? 0,
        y: origin?.y ?? 0,
        z: origin?.z ?? 0,
...
export function toGPUTexelCopyBufferLayout(layout) {
    return {
...
export function assertSampleCount(sampleCount, context) {
...
//# sourceMappingURL=wgpuEnumMap.js.map