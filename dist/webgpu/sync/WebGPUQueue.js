...
import { toGPUExtent3D, toGPUOrigin3D, toGPUTexelCopyBufferLayout, toGPUTextureAspect, } from '../utils/wgpuEnumMap.js';
export class WebGPUQueue {
    native;
    device;
    constructor(device) {
        this.device = device;
    ...
    writeBuffer(buffer, bufferOffset, data, dataOffset = 0, size) {
        ...
        if (bufferOffset < 0 || byteSize < 0 || bufferOffset + byteSize > buffer.size) {
            throw new ValidationError(`[gpu-device-api] writeBuffer 越界：写入范围 [${bufferOffset}, ${bufferOffset + byteSize}) ` +
        ...
        const bytesPerElement = data instanceof DataView
            ? 1
            : (data.BYTES_PER_ELEMENT ?? 1);
        if (dataOffset % bytesPerElement !== 0 || byteSize % bytesPerElement !== 0) {
            throw new ValidationError(`[gpu-device-api] Queue.writeBuffer: dataOffset (${dataOffset}) and size (${byteSize}) are measured in ` +
                `bytes, so both must be multiples of the element size (${bytesPerElement}) of the given ${data.constructor.name}.`);
        }
        this.native.writeBuffer(asGPUBuffer(buffer, 'Queue.writeBuffer'), bufferOffset, toAllowSharedBufferSource(data), dataOffset / bytesPerElement, byteSize / bytesPerElement);
    }
    /** 将主机端像素上传到 texture。多行/多层拷贝必须给 `layout.bytesPerRow`（WebGPU 会校验）。 */
    writeTexture(destination, data, layout, size) {
        this.device.assertUsable('Queue.writeTexture');
        this.native.writeTexture(toGPUTexelCopyTextureInfo(destination, 'Queue.writeTexture'), toAllowSharedBufferSource(data), toGPUTexelCopyBufferLayout(layout), toGPUExtent3D(size));
    }
    /**
    ...
    copyExternalImageToTexture(source, destination, copySize, flipY = false, options) {
         ...
         */
        const destinationInfo = toGPUTexelCopyTextureInfo(destination, 'Queue.copyExternalImageToTexture');
        ...
        this.native.copyExternalImageToTexture({ source, flipY }, destinationInfo, toGPUExtent3D(copySize));
    }
    /**
     * buffer → buffer 的拷贝。
     *
    ...
    copyBufferToBuffer(source, sourceOffset, destination, destinationOffset, size) {
        ...
        encoder.copyBufferToBuffer(asGPUBuffer(source, 'Queue.copyBufferToBuffer(source)'), sourceOffset, asGPUBuffer(destination, 'Queue.copyBufferToBuffer(destination)'), destinationOffset, size);
    ...
    copyBufferToTexture(source, destination, copySize) {
        ...
        }, toGPUTexelCopyTextureInfo(destination, 'Queue.copyBufferToTexture(destination)'), toGPUExtent3D(copySize));
        this.native.submit([encoder.finish()]);
    }
    /**
 ...
 */
function toAllowSharedBufferSource(data) {
    return data;
}
//# sourceMappingURL=WebGPUQueue.js.map