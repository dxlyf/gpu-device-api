 ...
 */
export function toGPURenderPassDescriptor(descriptor, device) {
    const label = descriptor.label ?? 'renderPass';
    let colorAttachments;
    let depthStencilAttachment;
    if (descriptor.target) {
        if (!(descriptor.target instanceof WebGPURenderTarget)) {
            throw new ValidationError(`[gpu-device-api] ${label}: descriptor.target must be a WebGPURenderTarget created by a WebGPU device.`);
        }
        /*
        ...
        if (descriptor.colorAttachments.length > 0) {
            throw new ValidationError(`[gpu-device-api] ${label}: descriptor.target and a non-empty descriptor.colorAttachments were ` +
    ...
    for (let index = 0; index < colorAttachments.length; index += 1) {
        ...
        if (texture.sampleCount > 1 && !attachment.resolveTarget && storeOp !== 'discard') {
            throw new ValidationError(`[gpu-device-api] ${label}: a multisampled color attachment (sampleCount ${texture.sampleCount}) needs a ` +
     ...
     */
    const native = { label, colorAttachments: nativeColors };
    ...
    if (depthStencilAttachment) {
        ...
        else if (depthStencilAttachment.stencilLoadOp !== undefined ||
            depthStencilAttachment.stencilStoreOp !== undefined) {
            throw new ValidationError(`[gpu-device-api] ${label}: depth format "${format}" has no stencil aspect, so stencilLoadOp / ` +
...
function assertConsistentSampleCount(sampleCounts, label) {
    if (sampleCounts.length === 0)
        return 1;
    const first = sampleCounts[0];
    for (const count of sampleCounts) {
        if (count !== first) {
            throw new ValidationError(`[gpu-device-api] ${label}: all attachments of a render pass must share the same sampleCount, got ` +
                `${sampleCounts.join(', ')}.`);
        }
    }
    return first;
}
function assertDepthClear(value, label) {
    if (!Number.isFinite(value) || value < 0 || value > 1) {
        throw new ValidationError(`[gpu-device-api] ${label}: depthClearValue must be within [0, 1], got ${String(value)}.`);
    }
    return value;
}
export class WebGPURenderPassEncoder {
    label;
    layout;
    native;
    device;
    ...
    constructor(device, native, layout, label, hasOcclusionQuerySet, onEnd) {
        this.device = device;
        this.native = native;
        this.layout = layout;
        this.label = label;
    ...
    setBindGroup(index, bindGroup, dynamicOffsets) {
        ...
        if (dynamicOffsets && dynamicOffsets.length > 0) {
            throw new ValidationError(`[gpu-device-api] RenderPass "${this.label}".setBindGroup: dynamicOffsets were given without a bind group.`);
        }
    ...
    setIndexBuffer(buffer, format, offset, size) {
        ...
        this.native.setIndexBuffer(asGPUBuffer(buffer, this.contextSetIndexBuffer), toGPUIndexFormat(format), offset, size);
    }
    ...
    draw(descriptor) {
        ...
        this.native.draw(descriptor.vertexCount, descriptor.instanceCount ?? 1, descriptor.firstVertex ?? 0, descriptor.firstInstance ?? 0);
    }
    drawIndexed(descriptor) {
        this.assertOpen('drawIndexed');
        this.native.drawIndexed(descriptor.indexCount, descriptor.instanceCount ?? 1, descriptor.firstIndex ?? 0, descriptor.baseVertex ?? 0, descriptor.firstInstance ?? 0);
    }
    ...
    assertOpen(context) {
        if (this._ended) {
            throw new ValidationError(`[gpu-device-api] RenderPass "${this.label}".${context}: the pass has already ended.`);
        }
    }
}
/** 把 `DrawIndirectDescriptor | BufferLike` 归一成 (buffer, offset)。 */
export function resolveIndirect(indirect, indirectOffset, context) {
...
//# sourceMappingURL=WebGPURenderPassEncoder.js.map