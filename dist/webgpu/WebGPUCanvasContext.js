...
export class WebGPUCanvasContext {
    ...
    configure(config) {
        ...
        if (!(config.device instanceof WebGPUDevice)) {
            throw new ValidationError('[gpu-device-api] CanvasContext.configure: expected a WebGPU device (WebGPUDevice).');
        }
        const context = getWebGPUCanvasContext(this.canvas);
        if (!context) {
            throw new ValidationError('[gpu-device-api] CanvasContext.configure: this canvas cannot create a WebGPU context ' +
         ...
         */
        const sampleCount = config.sampleCount ?? (this.configuredValue ? this.sampleCountValue : config.device.defaultSampleCount);
        ...
        if (this.colorSpaceValue !== undefined)
            configuration.colorSpace = this.colorSpaceValue;
    ...
    setSize(width, height, updateStyle = true) {
        if (!Number.isFinite(width) || !Number.isFinite(height) || width <= 0 || height <= 0) {
            throw new ValidationError(`[gpu-device-api] CanvasContext.setSize: width and height must be positive, got ${width}x${height}.`);
        }
    ...
    setPixelRatio(ratio) {
        if (!Number.isFinite(ratio) || ratio <= 0) {
            throw new ValidationError(`[gpu-device-api] CanvasContext.setPixelRatio: ratio must be positive, got ${String(ratio)}.`);
        }
        if (ratio === this.pixelRatioValue)
            return;
    ...
    getCurrentFrameTarget() {
        ...
        if (!this.configuredValue || !this.gpuContext || !device) {
            throw new ValidationError('[gpu-device-api] CanvasContext.getCurrentFrameTarget: the context is not configured.');
        }
        const native = this.gpuContext.getCurrentTexture();
        if (this.frameTarget && this.frameNative === native)
            return this.frameTarget;
        this.releaseFrame();
        const texture = WebGPUTexture.adopt(device, native, {
    ...
    /* ------------------------------------------------------------------ 内部 -------------- */
    ensureMultisampleTarget(width, height) {
        const device = this.currentDevice;
        if (!device) {
...
export function isWebGPUCanvasContext(value) {
    return value instanceof WebGPUCanvasContext;
}
//# sourceMappingURL=WebGPUCanvasContext.js.map