...
import { assertSamplerBindingType, toGPUBufferBindingType, toGPUSamplerBindingType, toGPUShaderStage, toGPUStorageTextureAccess, toGPUTextureSampleType, toGPUTextureViewDimension, } from '../utils/wgpuEnumMap.js';
...
export class WebGPUBindGroupLayout {
    ...
    constructor(device, descriptor) {
        ...
        try {
            ...
            throw new ValidationError(`[gpu-device-api] BindGroupLayout "${this.label}": ${error instanceof Error ? error.message : String(error)}`);
        }
        this.sortedEntries = sorted;
        this.entries = descriptor.entries;
        this.byBinding = new Map(sorted.map((entry) => [entry.binding, entry]));
        const entries = sorted.map((entry) => toGPUBindGroupLayoutEntry(entry, device, this.label));
        // bind group 数量上限由 device limit 约束，先按显式 layout 的数量做一次友好报错。
        if (entries.length > device.limits.maxBindingsPerBindGroup) {
            throw new ValidationError(`[gpu-device-api] BindGroupLayout "${this.label}": ${entries.length} entries exceed ` +
...
function toGPUBindGroupLayoutEntry(entry, device, layoutLabel) {
    ...
    if (isBufferBinding(entry.type)) {
        const layout = entry.buffer ?? {};
        if (layout.type !== undefined && layout.type !== entry.type) {
            throw new ValidationError(`[gpu-device-api] ${context}: buffer.type "${layout.type}" contradicts the entry type "${entry.type}".`);
        }
        native.buffer = {
    ...
    if (entry.type === BindingType.Texture) {
        ...
        if (sampleType === 'depth' && (viewDimension === '1d' || viewDimension === '3d')) {
            throw new ValidationError(`[gpu-device-api] ${context}: sampleType "depth" cannot be combined with viewDimension ` +
    ...
    if (entry.type === BindingType.StorageTexture) {
        const layout = entry.storageTexture;
        if (!layout || layout.format === undefined) {
            throw new ValidationError(`[gpu-device-api] ${context}: a storage-texture entry needs \`storageTexture.format\`.`);
        }
    ...
    throw new ValidationError(`[gpu-device-api] ${context}: unsupported BindingType "${String(entry.type)}".`);
}
/** 该对象是否为 WebGPU 后端的 bind group layout。 */
export function isWebGPUBindGroupLayout(value) {
    return value instanceof WebGPUBindGroupLayout;
}
/** 把任意 bind group layout 表示收窄为原生 `GPUBindGroupLayout`。 */
export function asGPUBindGroupLayout(value, context) {
    if (value instanceof WebGPUBindGroupLayout)
        return value.native;
    if (isNativeGpuObject(value))
        return value;
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU bind group layout (WebGPUBindGroupLayout or a native ` +
        'GPUBindGroupLayout).');
}
/**
 ...
 */
export function isNativeGpuObject(value) {
