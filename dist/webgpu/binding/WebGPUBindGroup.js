...
import { isFilterableFormat, textureFormatCapabilities, toGPUTextureFormat, } from '../utils/wgpuFormatMap.js';
    ...
    constructor(device, descriptor) {
        ...
        for (const entry of descriptor.entries) {
            if (!this.layout.entry(entry.binding)) {
                throw new ValidationError(`[gpu-device-api] BindGroup "${this.label}": binding ${entry.binding} is not declared by layout ` +
                    `"${this.layout.label}".`);
            }
        }
        const nativeLayout = asGPUBindGroupLayout(this.layout, `BindGroup "${this.label}"`);
        const entries = descriptor.entries.map((entry) => toGPUBindGroupEntry(entry, this.layout, device, this.label));
...
function toGPUBindGroupEntry(entry, layout, device, groupLabel) {
    ...
    if (isBufferBinding(layoutEntry.type)) {
        if (!('buffer' in resource)) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout declares a "${layoutEntry.type}" buffer binding, but the ` +
        ...
        if (offset + size > resource.buffer.size) {
            throw new ValidationError(`[gpu-device-api] ${context}: binding range [${offset}, ${offset + size}) exceeds the buffer size ` +
                `${resource.buffer.size}.`);
        }
        const minBindingSize = layoutEntry.buffer?.minBindingSize ?? 0;
        if (minBindingSize > 0 && size < minBindingSize) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout requires minBindingSize ${minBindingSize}, got ${size}.`);
        }
        const native = { buffer, offset, size };
        return { binding: entry.binding, resource: native };
    }
    if (isSamplerBinding(layoutEntry.type)) {
        if (!('sampler' in resource)) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout declares a "${layoutEntry.type}" sampler binding, but the ` +
                `resource is ${describeUnknown(resource)}.`);
        }
        const sampler = resource.sampler;
        const wantsComparison = (layoutEntry.sampler?.type ??
        ...
        if (isWebGPUSampler(sampler)) {
            if (wantsComparison && !sampler.isComparison) {
                throw new ValidationError(`[gpu-device-api] ${context}: layout declares a comparison sampler, but the bound sampler has no ` +
                    '`compare` function.');
            }
            if (!wantsComparison && sampler.isComparison) {
                throw new ValidationError(`[gpu-device-api] ${context}: layout declares a filtering/non-filtering sampler, but the bound ` +
    ...
    if (layoutEntry.type === 'texture') {
        if (!('view' in resource)) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout declares a "texture" binding, but the resource is ` +
    ...
    if (layoutEntry.type === 'storage-texture') {
        if (!('view' in resource)) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout declares a "storage-texture" binding, but the resource is ` +
                `${describeUnknown(resource)}.`);
        }
        if (isWebGPUTextureView(resource.view)) {
            ...
            if (expected !== undefined && toGPUTextureFormat(resource.view.format) !== toGPUTextureFormat(expected)) {
                throw new ValidationError(`[gpu-device-api] ${context}: layout requires storage texture format "${expected}", but the bound ` +
    ...
    if ('source' in resource) {
         ...
         */
        const external = resource.source;
        if (external && typeof external === 'object' && external.expired === true) {
            throw new ValidationError(`[gpu-device-api] ${context}: the external texture bound as { source } has expired. ` +
    ...
    throw new ValidationError(`[gpu-device-api] ${context}: unsupported binding resource ${describeUnknown(resource)}.`);
}
function validateTextureBinding(view, layoutEntry, device, context) {
    if (!isWebGPUTextureView(view))
        return;
    ...
    if (expected === 'depth') {
        if (capabilities.sampleScalar !== 'depth') {
            throw new ValidationError(`[gpu-device-api] ${context}: layout expects a depth texture, but the bound view has format "${format}".`);
        }
    }
    else if (expected === 'uint' || expected === 'sint') {
        if (capabilities.sampleScalar !== expected) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout expects a "${expected}" sample type, but the bound view has format ` +
                `"${format}" (${capabilities.sampleScalar}).`);
        }
    }
    else {
        if (capabilities.sampleScalar !== 'float') {
            throw new ValidationError(`[gpu-device-api] ${context}: layout expects a float sample type, but the bound view has format ` +
                `"${format}" (${capabilities.sampleScalar}).`);
        }
        if (expected === 'float' && !isFilterableFormat(format, device.features)) {
            throw new ValidationError(`[gpu-device-api] ${context}: layout declares sampleType "float" (filterable), but "${format}" is not ` +
    ...
    if ((layout.multisampled ?? false) !== multisampled) {
        throw new ValidationError(`[gpu-device-api] ${context}: layout declares multisampled=${String(layout.multisampled ?? false)}, but ` +
    ...
    if (expectedDimension !== view.descriptor.dimension) {
        throw new ValidationError(`[gpu-device-api] ${context}: layout declares viewDimension "${expectedDimension}", but the bound view ` +
 ...
 */
export const NO_DYNAMIC_OFFSETS = Object.freeze([]);
const dynamicOffsetSummaries = new WeakMap();
function dynamicOffsetSummary(layout, device) {
    ...
    for (const entry of layout.sortedEntries) {
        ...
        alignments.push(uniform ? device.limits.minUniformBufferOffsetAlignment : device.limits.minStorageBufferOffsetAlignment);
        limitNames.push(uniform ? 'minUniformBufferOffsetAlignment' : 'minStorageBufferOffsetAlignment');
    }
    const summary = { count: alignments.length, alignments, limitNames };
 ...
 * 校验错误，因此在录制阶段就报出来。
 */
export function validateDynamicOffsets(bindGroup, dynamicOffsets, device, context) {
    const summary = dynamicOffsetSummary(bindGroup.layout, device);
    if (summary.count === 0) {
        if (dynamicOffsets !== undefined && dynamicOffsets.length > 0) {
            throw new ValidationError(`[gpu-device-api] ${context}: bind group "${bindGroup.label}" has no entry with hasDynamicOffset, but ` +
                `${dynamicOffsets.length} dynamic offset(s) were supplied.`);
        }
        return;
    }
    if (dynamicOffsets === undefined || dynamicOffsets.length !== summary.count) {
        throw new ValidationError(`[gpu-device-api] ${context}: bind group "${bindGroup.label}" needs ${summary.count} dynamic ` +
    ...
    for (let i = 0; i < summary.count; i++) {
        ...
        if (!Number.isInteger(offset) || offset < 0) {
            throw new ValidationError(`[gpu-device-api] ${context}: dynamic offset #${i} must be a non-negative integer, got ${String(offset)}.`);
        }
        if (offset % alignment !== 0) {
            throw new ValidationError(`[gpu-device-api] ${context}: dynamic offset #${i} (${offset}) must be a multiple of ${alignment} ` +
                `(${summary.limitNames[i]}).`);
        }
    }
}
/** 该对象是否为 WebGPU 后端的 bind group。 */
export function isWebGPUBindGroup(value) {
 ...
 */
export function asGPUBindGroup(value, context) {
    if (value instanceof WebGPUBindGroup)
        return value.native;
    ...
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU bind group (WebGPUBindGroup or a native GPUBindGroup).`);
}
