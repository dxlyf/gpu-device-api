...
import { DEFAULT_PREWARM_TIMEOUT_MS, createCompilationMessage, describeCompilationInfo, mergeCompilationInfo, nowMs, withTimeout, } from '../../core/pipeline/CompilationInfo.js';
...
const EMPTY_VERTEX_LAYOUTS = [];
const EMPTY_COLOR_FORMATS = [];
/**
 * 变体解析二级缓存的容量。
 *
...
export class WebGPURenderPipeline {
    ...
    sampleCountContext;
    _disposed = false;
    warnedMissingVertexLayouts = false;
    ...
    constructor(device, descriptor) {
         ...
         */
        const resolved = resolvePipelineLayoutLike(descriptor.layout, (bindGroupLayouts) => this.device.createPipelineLayout({
        ...
        this.cache = new WgpuPipelineCache(64, (_value, key) => {
     ...
     */
    rememberVariant(input, resolved, key) {
        const memo = this.variantMemo;
        memo.unshift({
            input,
        ...
        if (memo.length > VARIANT_MEMO_LIMIT)
            memo.length = VARIANT_MEMO_LIMIT;
    }
     ...
     */
    async prewarm(variant = EMPTY_VARIANT, options = {}) {
        const started = nowMs();
        if (this._disposed) {
            throw new ValidationError(`[gpu-device-api] RenderPipeline "${this.label}" has been disposed.`);
        }
        ...
        try {
            if (typeof createAsync === 'function') {
                created = await withTimeout(createAsync.call(device, nativeDescriptor), options.timeoutMs ?? DEFAULT_PREWARM_TIMEOUT_MS, `createRenderPipelineAsync("${this.label}")`);
            }
            else {
     ...
     */
    async getCompilationInfo(_variant = EMPTY_VARIANT) {
    ...
    async collectCompilationInfo(failure) {
        ...
        const collect = async (module, stage, context) => {
        ...
        if (fragmentState) {
            await collect(fragmentState.module, ShaderStage.Fragment, `RenderPipeline "${this.label}".fragment.module`);
        }
    ...
    dispose() {
        ...
        if (this.ownsLayout && this.layout !== 'auto')
            this.layout.dispose();
        this.device.untrack(this);
    }
    resolveVariant(partial) {
        const descriptor = this.descriptor;
        const colorFormats = partial.colorFormats ?? this.defaultColorFormats();
        const sampleCount = assertSampleCount(partial.sampleCount ?? descriptor.multisample?.count ?? descriptor.render?.multisample?.count ?? 1, this.sampleCountContext);
        const depthFormat = partial.depthFormat !== undefined
            ? partial.depthFormat
        ...
        if (descriptor.fragment && !hasColorOutput(colorFormats)) {
            throw new ValidationError(`[gpu-device-api] RenderPipeline "${this.label}" has a fragment stage but no color formats. ` +
        ...
        if (!descriptor.fragment && (depthFormat === null || !WebGPURenderState.usesDepthStencil(depthState))) {
            throw new ValidationError(`[gpu-device-api] RenderPipeline "${this.label}" has neither a fragment stage nor a depthStencil ` +
        ...
        if (vertexLayouts.length === 0 && !this.warnedMissingVertexLayouts) {
            ...
            this.logger.debug('building with no vertex buffer layouts; declare `vertex.buffers` if the vertex shader reads attributes');
        }
        ...
        if (depthFormat !== null)
            toGPUTextureFormat(depthFormat);
    ...
    toGPURenderPipelineDescriptor(variant) {
        const descriptor = this.descriptor;
        ...
        let fragment;
        if (fragmentState) {
            const fragmentModule = asWebGPUShaderModule(fragmentState.module, `RenderPipeline "${this.label}".fragment.module`);
            fragment = {
...
export function isWebGPURenderPipeline(value) {
 ...
 */
export function asGPURenderPipeline(value, context, variant) {
    if (value instanceof WebGPURenderPipeline)
        return value.resolve(variant);
    const native = value?.native;
    if (native && typeof native === 'object' && typeof native.getBindGroupLayout === 'function') {
        return native;
    }
    throw new ValidationError(`[gpu-device-api] ${context}: expected a WebGPU render pipeline (WebGPURenderPipeline or a native ` +
        `GPURenderPipeline).`);
}
/** 供 cache key 诊断使用：只列出会参与 key 的字段。 */
export function describeRenderPipelineVariant(variant) {
 ...
 */
function hasColorOutput(formats) {
