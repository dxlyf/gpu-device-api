...
import { DEFAULT_BLEND_COMPONENT, DEFAULT_DEPTH_STATE, DEFAULT_PRIMITIVE_STATE, STENCIL_FACE_DEFAULT, } from '../../core/pipeline/RenderState.js';
import { validateVertexBufferLayout } from '../../core/pipeline/VertexLayout.js';
import { assertSampleCount, isStripTopology, toGPUBlendFactor, toGPUBlendOperation, toGPUColorWriteMask, toGPUCompareFunction, toGPUCullMode, toGPUFrontFace, toGPUIndexFormat, toGPUPrimitiveTopology, toGPUStencilOperation, toGPUVertexFormat, toGPUVertexStepMode, } from '../utils/wgpuEnumMap.js';
...
export const DEFAULT_STRIP_INDEX_FORMAT = 'uint32';
export class WebGPURenderState {
    ...
    constructor(state) {
        if (!state)
            return;
        if (state.primitive)
            this.primitive = state.primitive;
        if (state.depthStencil)
            this.depthStencil = state.depthStencil;
        if (state.multisample)
            this.multisample = state.multisample;
        if (state.blend)
            this.blend = state.blend;
        if (state.writeMask !== undefined)
            this.writeMask = state.writeMask;
        if (state.colorFormats)
            this.colorFormats = state.colorFormats;
    }
    /** 把 core 的 `PrimitiveState` 翻译为 WebGPU 的 `GPUPrimitiveState`。 */
    static toGPUPrimitiveState(state, features) {
        ...
        if (isStripTopology(topology)) {
            ...
            throw new ValidationError('[gpu-device-api] PrimitiveState.stripIndexFormat is only valid for strip topologies, but the topology ' +
                `is "${topology}".`);
        }
        if (state?.unclippedDepth) {
            if (features && !features.has('depth-clip-control')) {
                throw new ValidationError('[gpu-device-api] PrimitiveState.unclippedDepth needs the "depth-clip-control" device feature.');
            }
     ...
     */
    static toGPUDepthStencilState(format, state) {
        const hasDepth = hasDepthAspect(format);
        const hasStencil = hasStencilAspect(format);
        if (!hasDepth && !hasStencil) {
            throw new ValidationError(`[gpu-device-api] DepthStencilState: "${format}" has neither a depth nor a stencil aspect.`);
        }
        const native = { format: toGPUTextureFormat(format) };
     ...
     */
    static toGPUColorTargets(formats, targets, defaults = {}) {
        // 有效槽位：尾部连续的空位没有格式，也就没有「必须声明 target」的义务。
        let effective = formats.length;
        while (effective > 0 && formats[effective - 1] === null)
            effective -= 1;
        if (targets && (targets.length < effective || targets.length > formats.length)) {
            throw new ValidationError(`[gpu-device-api] RenderPipeline: fragment.targets has ${targets.length} entries but the render target ` +
        ...
        return formats.map((format, index) => {
            ...
            if (format === null) {
                if (target) {
                    throw new ValidationError(`[gpu-device-api] RenderPipeline: fragment.targets[${index}] declares a color target at ` +
            ...
            if (target === null)
                return null;
            const native = { format: toGPUTextureFormat(target?.format ?? format) };
            const blend = target?.blend ?? defaults.blend;
            if (blend)
                native.blend = WebGPURenderState.toGPUBlendState(blend);
            const writeMask = target?.writeMask ?? defaults.writeMask;
            if (writeMask !== undefined)
                native.writeMask = toGPUColorWriteMask(writeMask);
            return native;
        });
    }
    /** 校验并翻译 vertex buffer layout 列表。 */
    static toGPUVertexBufferLayouts(layouts, limits) {
        if (layouts.length > limits.maxVertexBuffers) {
            throw new ValidationError(`[gpu-device-api] RenderPipeline: ${layouts.length} vertex buffer layouts exceed maxVertexBuffers ` +
        ...
        return layouts.map((layout) => {
            try {
                ...
                throw new ValidationError(`[gpu-device-api] RenderPipeline: ${error instanceof Error ? error.message : String(error)}`);
            }
            return {
...
function toGPUBlendComponent(component) {
...
function toGPUStencilFaceState(face) {
    return {
...
//# sourceMappingURL=WebGPURenderState.js.map