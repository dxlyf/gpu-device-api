 ...
 */
import { cacheKey, createPipelineCache } from '../../core/pipeline/PipelineCache.js';
import { vertexBufferLayoutsKey } from '../../core/pipeline/VertexLayout.js';
export class PipelineCache {
    cache;
    created = new Set();
    _disposed = false;
    constructor(limit = 64, onEvict) {
        this.cache = createPipelineCache(limit, (value, key) => {
...
export function createWgpuPipelineCache(limit = 64, onEvict) {
    return new PipelineCache(limit, onEvict);
}
/**
 ...
 * 所以既有变体键不会因为这次变化而重排。
 */
export function colorFormatsKey(formats) {
    let key = '';
 ...
 */
export function renderPipelineCacheKey(variant) {
    return cacheKey(colorFormatsKey(variant.colorFormats), variant.sampleCount, variant.depthFormat ?? 'none', vertexBufferLayoutsKey(variant.vertexLayouts));
}
/** compute pipeline 只需要 entry point + layout，key 主要用于诊断。 */
export function computePipelineCacheKey(label, layoutKey) {
    return cacheKey(label, layoutKey);
}
/** 供调试输出：把 vertex layout 列表还原成可读文本。 */
export function describeVertexLayouts(layouts) {
