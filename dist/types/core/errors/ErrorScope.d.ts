...
export interface ErrorScopeHandle {
     ...
     *
     * `pop()` 之前可以安全读取。长度大于 1 是正常的：GL 的错误队列只保留一条，驱动在处理一次
     * 调用时可能覆盖多次，但本层一旦读到就会记账。
     *
     * ⚠️ 与 {@link ErrorScopeHandle.pop} 的返回值**不是一回事**：`errors` 是「这一层看到过什么」，
     * `pop()` 才是「谁该为它负责」。一条错误的 `filter` 与本层不同类时，它会被记进 `errors`
     * 但由更外层的作用域交出去（或作为未捕获错误走 `onError`）。
     */
    readonly errors: readonly GpuError[];
    /**
     * 弹出本作用域，返回**第一条**（最内层语义下唯一会被返回的那条）错误；本层无错或
     * 第一条错误的 `filter` 与本层不同类（且已被更外层接走）时返回 `null`。
     *
     * 与 `device.popErrorScope()` 完全等价 —— 内部走的是同一条路径，不存在「两套状态机」。
     * 没有交出去的错误不会被静默吞掉：它们要么继续交给更外层的作用域，要么作为**未捕获错误**
     * 走 `Device.onError`。
     *
     * 设备在它还没被 `pop` 时就被 `dispose()` 了的话，这里**拒绝**而不是返回 `null`
     *（设备已销毁、错误队列再也读不到，返回 `null` 等于说「作用域里没有错误」，那是撒谎）。
     */
    pop(): Promise<GpuError | null>;
}
/** `filter` 的合法取值表（运行时校验用，顺序即文档里的分类顺序）。 */
export declare const ERROR_SCOPE_FILTERS: readonly ErrorScopeFilter[];
/**
 ...
 */
export declare function isErrorScopeFilter(value: unknown): value is ErrorScopeFilter;
/**
 ...
 */
export declare function assertErrorScopeFilter(value: unknown, context: string): asserts value is ErrorScopeFilter;
//# sourceMappingURL=ErrorScope.d.ts.map