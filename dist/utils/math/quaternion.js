...
export function premultiply(out, a, b) {
 ...
 * 退化矩阵（全 0）返回单位四元数。
 */
export function setFromRotationMatrix(out, m) {
 ...
 */
export function setFromUnitVectors(out, from, to) {
    const fx = from[0];
    const fy = from[1];
    const fz = from[2];
    const tx = to[0];
...
/** 用四元数旋转向量：`out = q * v * q⁻¹`。 */
export function transformVec3(out, a, v) {
    const x = v[0];
    const y = v[1];
    const z = v[2];
    const qx = a[0];
