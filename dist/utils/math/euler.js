 ...
 */
import { multiply as multiplyQuat, setAxisAngle, toMat4 as quatToMat4, } from './quaternion.js';
import { fromXRotation, fromYRotation, fromZRotation, identity, multiply as multiplyMat4 } from './mat4.js';
/** 支持的顺序，按「常用程度」排列。 */
export const EULER_ORDERS = ['XYZ', 'YXZ', 'ZXY', 'ZYX', 'YZX', 'XZY'];
export function create(x = 0, y = 0, z = 0, order = 'XYZ') {
    return { x, y, z, order };
}
...
export function set(out, x, y, z, order = out.order) {
    out.x = x;
    out.y = y;
    out.z = z;
    out.order = order;
...
function axisRadians(e, axis) {
    return axis === 'X' ? e.x : axis === 'Y' ? e.y : e.z;
}
function axisRotationMatrix(axis, radians, out) {
    if (axis === 'X')
        return fromXRotation(out, radians);
    if (axis === 'Y')
        return fromYRotation(out, radians);
 ...
 */
export function toQuaternion(out, e) {
    const axes = AXIS_ORDER[e.order];
 ...
 */
export function fromRotationMatrix(out, m) {
    ...
    const m23 = m[9];
    const m33 = m[10];
    const clampUnit = (value) => (value < -1 ? -1 : value > 1 ? 1 : value);
