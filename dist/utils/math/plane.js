...
export function create(normalX = 0, normalY = 0, normalZ = 1, constant = 0) {
...
export function set(out, normal, constant) {
...
export function setComponents(out, x, y, z, constant) {
    out.normal[0] = x;
    out.normal[1] = y;
    out.normal[2] = z;
    out.constant = constant;
...
export function setFromNormalAndCoplanarPoint(out, normal, point) {
 ...
 */
export function setFromCoplanarPoints(out, a, b, c) {
...
/** 把点投影到平面上。 */
export function projectPoint(out, a, point) {
...
export function coplanarPoint(out, a) {
 ...
 */
export function intersectLineSegment(a, start, end) {
...
export function applyMat4(out, a, m) {
    ...
    if (!normalMatrix) {
        throw new RangeError('[gpu-device-api] plane.applyMat4: the matrix is singular, so the transformed plane is undefined.');
    }
    ...
    const length = Math.hypot(out.normal[0], out.normal[1], out.normal[2]);
    if (length < DEGENERATE_LENGTH) {
        throw new RangeError('[gpu-device-api] plane.applyMat4: the transformed normal is degenerate (the matrix collapses the plane).');
    }
    out.normal[0] = out.normal[0] / length;
    out.normal[1] = out.normal[1] / length;
    out.normal[2] = out.normal[2] / length;
