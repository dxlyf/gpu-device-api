...
/** 按列主序逐个写入 16 个分量。 */
export function set(out, ...values) {
    for (let i = 0; i < 16; i++)
        out[i] = values[i] ?? 0;
    return out;
}
 ...
 */
export function multiplyAll(out, ...matrices) {
    if (matrices.length === 0)
        return identity(out);
    copy(out, matrices[0]);
    for (let i = 1; i < matrices.length; i++)
        multiply(out, out, matrices[i]);
    return out;
}
/** 由平移量构造矩阵。 */
...
export function fromRotationTranslation(out, rad, axis, translation) {
...
export function fromRotationTranslationScale(out, rad, axis, translation, scale) {
    let x = axis[0];
    let y = axis[1];
    let z = axis[2];
    let len = Math.hypot(x, y, z);
 ...
 */
export function fromRotationTranslationScaleOrigin(out, rad, axis, translation, scale, origin) {
...
export function getScaling(out, a) {
...
export function getRotation(out, a) {
 ...
 * ```
 */
export function flipClipY(out, a) {
    if (out !== a)
        out.set(a);
    out[1] = -out[1];
    out[5] = -out[5];
 ...
 */
export function transformPoint(out, m, v) {
    const x = v[0];
    const y = v[1];
    const z = v[2];
...
export function toString(a) {
    ...
    for (let row = 0; row < 4; row++) {
        rows.push(`[${a[row]}, ${a[row + 4]}, ${a[row + 8]}, ${a[row + 12]}]`);
    }
