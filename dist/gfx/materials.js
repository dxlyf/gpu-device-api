 ...
 * `renderer.createMaterial(...)` 或 `defineMaterial(...)` 使用：
 *
 * ```ts
 ...
 * `renderer.draw(geometry, { uniforms: { ... } })` 或 `material.createUniforms()` 覆盖。
 *
 * 每种材质的 GLSL 与 WGSL 都成对写好，**访问名保持一致**（`u.*` 与 `v.*`），
 * 所以同一份材质描述在 WebGL2 与 WebGPU 上表现相同。
 */
 ...
 *
 * 与其它内置材质的区别：它读 `uv` 作为「衰减因子」（由 `createGrid`/`createAxes` 生成的
 * 顶点色提供），所以属性只有一个 `position` 与一个 `uv`。
 */
export function flatLine(options = {}) {
...
/**
 * 带顶点色的线段材质：顶点色来自几何体的 `color` 属性。
 * `createGrid` / `createAxes` 生成的数据就是给它的（远端渐隐、坐标轴三色）。
 */
export function vertexColorLine() {
    return {
...
export function defaultUniformsOf(desc) {
