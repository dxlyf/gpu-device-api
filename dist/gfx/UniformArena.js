...
export class UniformArena {
    ...
    retired = [];
    _disposed = false;
    constructor(device, layout, options = {}) {
        this.device = device;
        this.layout = layout;
    ...
    write(values) {
        ...
        if (values.layout !== this.layout) {
            throw new ValidationError(`[gpu-device-api] uniform arena「${this.label}」的布局是「${this.layout.structName}」，` +
    ...
    bindGroup(layout) {
        ...
        if (this.bindGroupValue && this.bindGroupLayoutValue === layout)
    ...
    allocate() {
        if (this.head + this.slotSize > this.capacityValue)
            this.grow();
    ...
    grow() {
        ...
        const next = Math.min(Math.max(this.capacityValue * 2, needed), this.maxCapacity);
        if (next < needed) {
            throw new ValidationError(`[gpu-device-api] uniform arena「${this.label}」本帧需要的容量超过了上限 ` +
...
//# sourceMappingURL=UniformArena.js.map